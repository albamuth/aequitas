...
> **Companions:** `Aequitas_Foundations_v0.22.md` §1.2/§9 (scope, conformance list), `Aequitas_Strategy_v0.5.md` §2, `06-simulation/scenario-suite/scenario_suite_METHOD.md`.
...
Foundations §9 now carries **17 requirements that must hold for an implementation to be Aequitas.** They were written for an implementer. **They are also exactly what the kernel should assert, every period, in every scenario.**
