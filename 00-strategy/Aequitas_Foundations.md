...
> **Companion:** [`Aequitas_Objections.md`](Aequitas_Objections.md) — the objections register.
...
> **This version puts pledges in the gate, and the gate was written wrong.** §3.0 said `D ≤ ρ·C` while §4.6 called a pledge *"permanent debit-room granted to other people"*, and **both could not be true.** The gate is **`D ≤ ρ·(C + P)`**, where `P` is room others pledged to a person by name. **ρ multiplies both terms**, or a pledged hour buys less room than a worked one, which is a rate distinction A2 forbids — and ρ would not reach accumulated room, leaving a network nothing to ration with. **A pledge now names a person, a fund, or the criteria that will name a person; it grants nothing until a person is named; once named the room is committed, consumable and unchangeable.** The contingent reserve is **withdrawn** and kept as a worked example of what a network rules on, which reaches the tedium half of OP-16 that it never could. **§5.5.5's ceiling is withdrawn over consumption and restated over pledge budgets** — same arithmetic, and it now bounds the say over what gets made, against money's 10⁶×. **Consumption has no per-person bound, and §5.5.6 used to infer one.** Five conditions instead of four: **room received must never raise a pledge budget.** §5.5.3's band gains the pledge term and **closes for the first time** at `F` = 1 with a 70% essentials basket; the ρ = 1.2 used throughout sits exactly at the publishable edge with no pledging and above it with any. **§2.6 gains two settings a network sets** — who may consent to a property transfer, and how a service's cost is attributed. **A child carries no debit**: care credits the carer and debits nobody, which §4.6 already said of any service. *(Rulings are the author's, 2026-09-18; worked in `papers/Pledges.md` and `../02-research/Problem_pledge-inflation.md`.)*
>
...
**Every debit is a record of a real material or energy flow. Every credit is a record of time a person spent.**

**No abstract, issued, or fiat quantity exists anywhere in the system.**

**A flow is attributed to whoever caused it. Only people act, so responsibility attaches to people, and never to a tool, a machine, or whoever owns one.**

*Where this is worked out: §2.2 for what a debit is and the grounding for attribution · §2.3 for what a credit is · §3.1 for why stocks, bonds and currencies never appear.*

<!-- tag: fnd-a2 -->
### A2 (time as measure)

**Time is a yardstick for summarising flows. It is not itself a substance with value.**

**Labour is never rate-scaled. One hour credits as one hour, whoever worked it.**

**Differences between workers — exertion, hazard, and skill — are recorded as material costs, never as a multiplier on time.**

*Where this is worked out: §2.3, with each of the three differences shown · §3.4a for why the unit of account never has to be chosen.*
...
**A credit can never be transferred, traded, gambled, lent, or stolen.**
...
**Possession decides who carries the debit that rides a thing. Taking a thing is taking on that debit; handing it on moves whatever moves with it.**
...
**There is no profit in exchange. There is only debit acquired and debit discharged.**

*Where this is worked out: §2.2 for how A5 and A1 divide the work · §2.4 for what a hand-off does · §3.2 for which parts of a debit move and which stay · §4.5 for why a barn is not in the beef · §5.1 for what competition is left.*

<!-- tag: fnd-a6 -->
### A6 (derived, not stored)

**An account's standing is never stored. It is computed from the event log whenever anyone asks, by applying the current weighting model to the events.**

**The log is authoritative. A balance is not.**

*Where this is worked out: §3.0 for what a standing is · §3.1 for the log · §3.3 for what happens when the science improves.*

<!-- tag: fnd-a7 -->
### A7 (universal accounting)

**Every human is in the books, whether or not they take part. Credit and debit are estimated for everyone.**

**An estimated position becomes usable only when the person holds a verified account and observed records have replaced the estimates.**

*Where this is worked out: §4.1 for what an estimate is and is not · §4.4 for how a network estimates what it cannot see, and for a life rebuilt back to birth.*
...
**The axioms above may not vary, and neither may the conformance requirements** — [`Aequitas_Conformance.md`](Aequitas_Conformance.md).
...
**"Matter or energy" means all of it, with nothing too small to record** — down to the oxygen a human breathes in and the carbon dioxide they breathe out.

#### Who a flow is attributed to, and why

**A1 says a flow attaches to whoever caused it. The reason is a principle courts already use everywhere else: impute responsibility to whoever was in fact responsible.**

**This is [David Ellerman's labour theory of property](http://et.worldeconomicsassociation.org/files/WEA-ET-5-1-Ellerman.pdf), and it is chosen deliberately over any labour theory of *value*.** A theory of value tries to say what things are worth. **A theory of imputation only says who did what**, so it inherits none of the problems that sank the value theories, and it argues from a principle its opponents already accept.
...
**A reader meeting both axioms will ask which one decides who holds a debit. They both do, for different debits.**
...
**Both halves are remediation processes, and §3.3 states the rule that governs them.** This section says why remediation is the quantity read, rather than one of the two things a reader will expect.
...
**This is §2.5's test, applied to a pollutant.** *Did the thing being divided leave a physical trace?* **A tonne of a substance in the air is a physical trace, and the work of taking it out is measurable.** A person's sense of loss is not, and no better instrument will ever make it so.

##### An example, with the numbers

Someone releases **1 tonne of CO₂**. Three ways to put a number on it. *(All three figures are illustrative.)*
...
**A credit belongs to one person and never moves, and that follows from A1 rather than being a design choice.** A credit records **who was responsible**. Responsibility is a fact about a person. **Facts about people do not change hands**, so there is nothing coherent to transfer (A3).

#### Everyone earns at the same rate

**There is no multiplier for a profession, a rank, a skill, or a hazard** (A2). **Real differences between workers are real, and every one of them lands as a material cost instead.**
...
**None of the three is a larger number for the same hour.**
...
**Which activities a network *always* credits — childcare, schooling and whose schooling, subsistence farming, untrained medical assistance — is not settled by this document and should not be.** It is exactly the kind of question A8 leaves to networks competing in public.
...
**Name the risk plainly: the list is a capture surface.** A network that can declare an activity creditable can issue credit. **The gate narrows what a network may declare. It does not close the question.**

**A network cannot declare loyalty to itself creditable**, because loyalty is not production, not a service, and not enrichment. **It can still stretch *enrichment* a long way**, and nothing here stops it.

**Three defences carry the rest, and all three are structural rather than procedural.**
...
**This is OP-10 (weighting governance) wearing different clothes and should be worked with it.**
...
**None of the three is a currency, so they never cancel each other.** Nothing is netted, nothing is spent, and **no fourth number is produced by subtracting one from another.** A ledger is a way of looking at one person's consumption beside their work done and the room others have put behind them, and that is the whole of it.

**Two things follow immediately, and the rest of §3 depends on both.**

**1. All three numbers only ever rise.** A purchase adds to `D` and takes nothing from `C`, because credit is not something you hand over (A3). A pledge adds to the recipient's `P` and is never taken back (§4.6). There is no stored lump to draw down and nothing to run out of.

**2. So they are compared as a ratio, never as a balance.** A trust network's consumption gate is:

> **`D ≤ ρ·(C + P)`**

where **ρ** (said "rho") is a multiplier the network sets, called its **debit tolerance** (§3.5, §5.5). At ρ = 1.2, a person may carry debit up to 1.2 times their recorded credit plus the room pledged to them.

**ρ multiplies both terms, and that is not a detail.** A2 says one hour credits as one hour, whoever worked it. **Write the gate `ρ·C + P` instead and a pledged hour buys less room than a worked one, which is a rate distinction between two hours.** It also puts pledged room beyond ρ's reach, leaving a network no instrument to ration with (§4.6, §5.5.3).

**`P` counts committed room only.** A pledge naming a fund, a cause or an organisation grants nothing to anybody until it names a person (§4.6).

**None of the three is stored anywhere.** All are recomputed from the event log whenever anyone asks (A6, §3.1), so a better weighting model changes `D` for everybody, backwards through history (§3.3).

##### An example, with the numbers

Three people, all aged 40, in a network with a floor of 10 hours a day and ρ = 1.2. **A median lifestyle commands about 1,380 hours of other people's labour a year** (§3.5).
...
**What is recorded is the *material* the claim is a claim upon** — a factory, a building, a piece of land. **That material's debit sits on whoever physically holds or operates it** (§3.2: the embodied material discharges on transfer, the making-hours split by holding time, §4.5). **It never sits on the paper.**

**This is not a way of hiding wealth.** Owning a factory through shares does not move its material debit to nobody. **It stays on the people who operate the factory, divided by how long each has been there.**
...
**Line 2, in three cases.** Transport fuel and its pollution stay with **whoever held the fuel parcel and burned it.** **A driver owns the tailpipe, not the carmaker.** And whoever burns the petrol owns the CO₂ that burning releases.

**Held the parcel, not caused the journey.** The party who benefits from a journey is not the party the books can read. **The party who held the fuel is** (§3.4a). So a shop that fuels its own van and delivers to customers carries every gram it burned, and **the customers carry none of it.** A customer who instead drives to the shop in their own car burned their own fuel, so that journey is theirs by the same rule and by no separate one.

**An organisation is not the end of the line.** Fuel bought by an organisation and burned by its members is debit on the organisation, which is **at all times its members' debit, divided by the hours each worked** (§3.2c). **It cannot be tagged onto the goods the journey carried.**
...
A grocery co-op of **ten members**, each working **2,000 hours** a year, buys **40 litres** of diesel and delivers **2,000 orders**. It sells 10 litres on to another haulier and burns the other 30. *(The 0.05 h/kg remediation rate is illustrative.)*
...
**In plain words: the debit moves while the fuel is still liquid and stops moving the moment it is burned.** Spreading the 3.47 hours across 2,000 orders would put a cost into goods that did not consume it, which A5 forbids.
...
**The scope is *acting for it*, and that phrase is doing work.** A member burning the organisation's fuel on the organisation's route caused a debit **jointly**, because the organisation's members decided together to run that route. **A member who burns their own fuel on their own errand caused it alone**, and §3.2 puts it on them alone.

**Why the division is needed at all.** A reader can object that only the driver's foot was on the pedal, so A1 (materialism of cost) puts the whole tailpipe on the driver. **That reads "the causer" as one pair of hands.** The route existed because ten people organised it, and every one of them acted. **Naming the driver alone would be picking the last person in the chain**, which is not what A1 says.
...
**A trust network cannot check arithmetic it is not allowed to read.** So a claim entered into the books — a credit claim, a hand-off, a figure a good carries, a passage claim off a plant's flow sheet — **is auditable by the network, and entering it is what makes it so.**
...
**Two conditions, and the leftover does not work without both.**

1. **An independently known `N` must exist.** It does for major commodities, and it does not for everything. **Where no `N` exists, this form is unavailable rather than weak**, and a fabricated `N` is the failure it invites.
2. **`N` and `Y` must measure the same quantity, over the same piece of the world, over the same stretch of time, with error bounds smaller than the difference between them.** Fail any one and the gap between them is an artefact of the mismatch rather than a real leftover.
...
**The leftover is continuous, not a single event.** As part of an area becomes measured, `Y` rises and `R` shrinks to what remains. **The parts must reconcile against the coarser figure they came from.** This is also what catches a fabricated total, because a fabricator does not control which part gets measured next.
...
**This was measured rather than argued.** `06-simulation/producer-side-splitting/` put two different worlds in front of one network. **Both give that network identical books to the last decimal place — the same `N`, the same `Y`, the same count of missing producers, the same `R` — while the truth behind them differs by a factor of 1.21.** No rule computed from those numbers can tell the two worlds apart.

**That is §4.4's own sorting rule arriving again:** *a check that compares a thing to itself can find a mistake, and it cannot find a hole.*

**And a head-count is the wrong kind of quantity to divide by.** §2.5 asks whether the thing being divided left a physical trace. **A region's unseen output left one — hectares, vessel-days, throughput measured by the same survey that produced `N`. A count of producers left none.** Dividing by the count says a 100-hectare farm and a 600-hectare farm each produced the same amount, which no instrument ever said.

#### A7 still puts every producer in the books, and §4.1 is where that estimate comes from

**A7 (universal accounting) requires an estimated position for every human whether they take part or not.** Withdrawing the per-head slice must therefore not leave an unsubscribed producer with no figure at all, **and it does not.**

**§4.1 already names the source, and always did:** *a production model for their occupation, region and known activity, computed while excluding measured producers.* **That model is where A7's estimate comes from.** The per-head slice was a second and cruder route to the same place, and it is the one that goes.
...
**Why that part cannot be dropped as well.** Computed over everybody, the rule rewards hiding: producers better than average install instruments to prove it and leave the pool, and producers worse than average stay unmeasured and receive an average their own absence pushed upward. **Computed over the leftover, the estimate gets worse for those who remain as the good ones leave. Staying unmeasured stops paying, and stops paying more the longer it lasts.**
...
**What this does not touch is the disparity ceiling.** Deleting a year removes days from the denominator **and the credit for those days from the numerator**, and IC-7 caps a removed day at 24 hours. **So `24 ÷ F` holds under a shortened span** (§5.5.5), and the exposure is to the consumption gate `D ≤ ρ·(C + P)` alone.
...
**The pledger's budget is spent at pledge time in every one of those rows**, and there is no path that returns it. **An undistributed pledge lapses**, so a fund that never distributes has spent its pledgers' budgets and created no room.

**Where a pledge states no criteria, room distributes by hours worked on the work it named.** That is not a preference. **A1 rules out the fund itself, because only people act. A2 rules out every basis that weighs one worker's hour above another's** — skill, rank, seniority, need. **And §4.4's rule against dividing by a head count rules out an equal split**, for the same reason: a head count leaves no physical trace. **Hours worked is what is left, and §3.2c already divides by it.**

**Criteria may be any conditional thing, including a judging panel.** A prize for solving a hard problem is exactly this: many people pledge toward a stated achievement, a panel decides who reached it, and the room commits to them. **No objective rule stated in advance could replace the panel**, because nobody can write criteria for *"solved a problem nobody has solved."*
...
**⚠️ This makes discretion visible and does not remove it.** A very large fund with discretionary published criteria is a coordinator with real power over who may consume. **That is P4 (coordinator class), the live blocker** — a new surface for it, not a new problem.

##### A pledge is an act, and it requires consent

**Nobody pledges by default and nobody pledges automatically.** A budget accrues to every living person from birth, because staying alive is credited work (§4.5). **Somebody who cannot consent cannot spend any of it**, and an infant is the clear case.

**Who can consent, and to what, is the network's decision** (§2.6).
...
**And that one-for-one backing is what bounds the whole system's room.** Committed room across a network can never exceed the credit its subscribers earned, so **total room reaches at most `2·ρ·C_total` and never more.** The factor is 2 at every value of ρ. **A network must therefore choose ρ knowing pledges can double its gate** (§5.5.3). **What that does to an economy, and why it cannot produce a price spiral, is worked in full in [`../02-research/Problem_pledge-inflation.md`](../02-research/Problem_pledge-inflation.md)** — the short form is that inflation needs a seller who raises the number, and A5 leaves no price to raise.
...
**The surplus is room like any other, and it commits to whoever the criteria name.** Where the criteria are silent it commits to the doers, by hours worked on that task.

**Aequitas does not decide what those criteria are, and a network does.** The example below is what a network would rule on, not a rule this document imposes.
...
**And the lever is far more evenly spread, which is measurable.** Money's top tail reaches about **10⁶ times** the median. **Pledging power per day lived cannot exceed 24 ÷ F**, which is about **2.4 times** at a ten-hour floor, **and that is an absolute maximum nobody reaches** (§5.5.5). **Every living person holds some.** **This is the bound the project states, and it is about the say over what gets made rather than about what anybody consumes** (§5.5.5).
...
**What survives is competition, and the system depends on it.** A5 removes margin, not rivalry.
...
**Doing the same thing with fewer hours, less material and less waste is the whole of the contest.** §3.3a leans on this directly, and says plainly why the lean is weaker than it looks: rival sectors auditing each other's cost constants is one pressure among several rather than a mechanism, and systemic under-costing is still open (**OP-24**).
...
**Why the arithmetic gives that.** A pledge budget equals the credit a person earned in their life and nothing else (conformance requirement 9). IC-7 caps any account at **24 hours of activity per 24 hours** (conformance requirement 8), and every living subscriber accrues at least `F` hours a day. **So every subscriber's budget per day lived sits between `F` and 24, and the ratio of the largest to the smallest is `24 ÷ F`.** At `F` = 10 h/day that is **2.4**.

##### Why it is not a bound on consumption

**Consumption is gated by `D ≤ ρ·(C + P)`, and `P` — the room other people pledged to a person — has no limit on any one account** (§3.0, §4.6). **So a much-backed person may consume far beyond `24 ÷ F` times a floor-only life, and this system does not prevent it.**
...
**That is an accepted property rather than a defect, and the reason is that the two kinds of wealth are not alike.** Room granted by pledges **cannot be lent at interest, rented out, charged for, or used to hire anybody**, because credit never moves (A3), nothing may be added to a cost figure (A5), and working on your own property raises that property's debit by the hours it credits (§3.2). **Property under capitalism is a machine for getting more. Pledged room is a one-time grant that lets a person hold and consume.**

**And it cannot be manufactured.** A budget is credit somebody earned, capped by IC-7 at 24 hours a day. **To hand somebody a million hours of room you need real, verified humans who each lived a life**, so concentration is bounded by how many people will back you and by nothing else.
...
5. **Receiving room must never raise a pledge budget.** A budget is credit the holder earned. **If room granted to a person enlarged what they could pledge, `P` would re-enter and this bound would fail exactly as the consumption claim did.**
...
**So `D ≤ ρ·(C + P)` is a ratio re-checked at every event, not a balance drawn down.** A person who consumes nothing for decades and then spends heavily can only bring forward their own allowance, which is bounded by `ρ·(C + P)`. **There is no stored lump to release.**

**At equal age, two people's cumulative credits stand in a ratio of at most `24 ÷ F`, and so do their pledge budgets.** The only spread beyond it is age — time lived, not class. **A 60-year maximum worker against a 20-year floor-only person is 3 × 2.40 = 7.20× on lifetime totals, and 2.40× per day lived.** Both figures describe the same two accounts; §5.5.5 states the bound per day lived, which is the form that holds without an age condition.
...
**Food, fuel, heat and water are used up when they are used.** Their debit is consumption debit, which §3.2 makes permanent on whoever caused it and which no transfer sheds (§4.7: *"They cannot trade their way out"*).

**So a large part of every ledger is a debit figure that only ever rises, with nothing held against it.** A person cannot stockpile the meals they ate.
...
`06-simulation/disparity-ceiling/disparity_ceiling_sim.py`, N = 200,000, gate `D ≤ ρ·(C + P)`, credit in `[F, 24]` h/day, 7 self-tests green:
...
`06-simulation/scenario-suite/q4_locked_ledgers.py` applies the gate to real US and world distributions under the **material-only** rule (A1, and the financial-instruments rule in §3.1), asking what fraction of people would sit past a permanent lockout — non-essential consumption held at the floor for life because their sustained footprint exceeds `ρ · 24 h/day`, **the most any human can earn on their own.**

- **Stripping the financial layer collapses the top of the distribution by about 1,000×.** Money wealth reaches ~10⁶× the median, but material **consumption** only ~670× (Oxfam billionaire personal footprints), because consuming physically takes bounded time. **The spread is far smaller than the monetary one.**
