...
> **Version:** 0.22
> **Date:** 2026-08-24
> **Status:** Working foundations. **Mid-batch — companions (Objections, Overview, GLOSSARY) are bumped once when findings #18 and OP-9 are also ruled.**
> **Primary audience of the first paper:** technologists / implementers.
> **Companion:** `00-strategy/Aequitas_Objections_v0.21.md` — the objections register. Read alongside §10.
...
**What may not vary.** The axioms above, and the conformance requirements in §9.
...
>
> **Two places in this document already depend on that.** §5.3c expects networks to **federate and merge toward a single network over time**, not to settle into separate regional systems. And the disparity ceiling of `24 ÷ F` holds **across any set of networks compatible enough to trade** (§7.5, condition 5) — a statement about the whole interoperating economy, not about one town.
>
...
**Three conditions.** It requires an independently known *N*, which exists for major commodities and not for everything; a defensible count *Z*, since under-counting dark producers over-states each one's share; and — the condition that makes the subtraction mean anything at all — ***N* and *Y* must measure the same quantity, over the same boundary, over the same window, with error bounds small enough that the difference between them is real.**

> ### 📦 WHY YOU MUST CHECK THE TWO NUMBERS MATCH — IN PLAIN WORDS
>
...
> - **Y** = 82,000 tonnes. Recorded by farms inside Aequitas.
> - **R = N − Y = 18,000 tonnes**, said to be grown by dark farms.
>
> Now check the four rows.
>
> | Check | What is actually true | Effect on R |
> |---|---|---|
> | Quantity | *N* is **harvested grain**. *Y* is **grain sold**. Farms kept 6,000 t for seed and feed. | R is **6,000 t too big** |
> | Boundary | The satellite covers the whole valley. Aequitas farms sit in the **upper valley only**. | Not comparable at all |
...
>
> **Read the last row on its own.** The residual is 18,000 tonnes and the doubt is 12,000 tonnes. **The true residual is somewhere between 6,000 and 30,000 tonnes.** A five-fold range is not a finding.
>
> **Now fix the four rows.** Use sold grain for both. Use the upper valley for both. Use the crop year for both. Use a survey with ±3% (±3,000 t) error.
>
> - **N** = 88,000 t ± 3,000
> - **Y** = 82,000 t
> - **R = 6,000 t ± 3,000** — between 3,000 and 9,000 tonnes.
>
> **That is a residual.** It is smaller, it is honest, and you can act on it.
>
> **⚠️ Note what happened to the number.** The unchecked residual was 18,000 t. The checked one is 6,000 t. **Skipping this check made the dark pool look three times larger than it is** — and every dark producer's estimated share with it.

**Where the check happens.** Both *N* and *Y* already carry the fields this needs — extent, vintage and error bounds are in the provenance block every estimated record must have (EventLog §4.1a). **The fields existed; the rule that they must agree before you subtract did not.** It is now stated as a precondition, and EventLog §7.2a carries it as a check.

> **The output form when the check fails.** A mismatch does not invalidate the claim; it **downgrades** it, exactly as the floor rule downgrades a partial count (§5.1a). Report the residual as **a lower bound plus the boundary and window actually observed**, never as an exact figure. **A conserved unknown is better than a balanced fiction** — and a residual is only ever attributed to a person on **attribution evidence**, never on membership of a cohort (§5.1c).
...
> ### 📦 THE ONE QUESTION THAT SORTS EVERY CHECK — IN PLAIN WORDS
>
...
>
> A check that compares two things made on separate paths can find a hole — **if** the second thing is expressive enough to say so (§4).
>
> #### An example, with the numbers
>
> A farm records 8 sacks in and 8 sacks out. Someone then deletes the last 2 sacks from **both** halves of the record.
>
> | Check | What it compares | Sum | Fires? |
> |---|---|---|---|
...
> | **The buyer's own receipt** | **A record on a second path** | **buyer holds 8, farm says 6** | ✅ **Yes — short by 2** |
>
> **The first three checks are arithmetic over one log. Cutting the log never breaks arithmetic over the log**, because what is left is still balanced. Only the fourth check reaches outside.
>
> **This is why the table above lists a witness for each flow type rather than a constraint.** A constraint reads the log. A witness was made somewhere the log's failure could not reach.
...
| Aequitas | Pledging power is bounded by **24 ÷ F ≈ 2.4 ×** at a 10-hour self-care floor (§7.5) |
...
> **Formally stated, simulated, and stress-tested → PASSES.** The formal statement + a plain-language explainer are in `06-simulation/disparity-ceiling/DISPARITY_CEILING.md`; the adversarial pass (2026-08-14) dissolved all three attacks — **Methuselah** (above), **dynasty/household** (a household is a co-op; its dwelling-debit holding-time-splits per occupant by dwelling-time, children included — the bound is **per-person**, and inheritance *dilutes* it, §6.2b), and **collector** (holdings are a burden that raise your own debit, so a hoard self-bounds). The agent-based model — `06-simulation/disparity-ceiling/disparity_ceiling_sim.py` (N = 200,000, gate `D_i ≤ ρ·C_i`, credit ∈ `[F,24]` h/day), **7 self-tests green** — shows, all within-model:
>
...
>
> **Still conditional on OP-22** — the sim *assumes* the anti-arbitrage guard is implementable (it does not model the disclosure mechanism), and the structural results (2.4×, ρ-independence, fraud-invariance) hold for any distribution while the *clearing-ρ\** values are illustrative. What remains for full closure is the like-for-like against real wealth micro-data (Objections §C test 8). This anchors on the median-lifestyle work (`06-simulation/median-lifestyle/MEDIAN_LIFESTYLE_RESULT.md`).

> **The real-distribution comparison, now run**. `06-simulation/scenario-suite/q4_locked_ledgers.py` applies the ceiling to real US/world distributions under the **material-only** rule (A1 corollary above), asking what fraction would sit past a *permanent* efficiency-ratio lockout — discretionary consumption pinned to the floor for life because their sustained footprint exceeds `ρ · 24 h/day`, the most any human can ever earn. Two results sharpen §7.5:
...
| Self-regulation by participants | Governance as **protocol property** (A8 (no governing body)). |
...
Ranked by how load-bearing they are. Full detail in `00-strategy/Aequitas_Objections_v0.21.md`.
...
- **OP-9 (calculation reply) — Preference revelation.** Answered by §6.4 pledges and §6.4d, plus scarcity-as-debit on the Kantorovich framing. *v0.22 adds the move the reply was missing:* **the objection assumes a price is an honest reading of demand, and in a concentrated market it is not** — supply is restricted and demand is manufactured by the same firms, which is Braudel's anti-market layer (§6.4d). **A pledge cannot be advertised into existence**, and the demand lever's concentration falls from money's ~10⁶× to the `24/F ≈ 2.4×` ceiling. **What stays open is not preference revelation but the objective function** where a scarcity constraint spans the whole economy — that is OP-10 business, registered there.
...
- **OP-25 (illicit dumping) — Illicit end-of-life dumping.** §3.6 prices *lawful* disposal correctly; abandoning an object in the environment to escape its end-of-life debit is a Level-2 trust-and-provenance attribution problem. *(The **repeat-shell entities** sibling added in v0.19 is **closed in v0.22** by §3.2c — an organisation's debit is its members' debit, so closing one moves nothing. What remains of it is a **fake or borrowed membership list**, which is a verification question under **C6 (identity)**, not an accounting one.)*

**Closed**
- ✅ ~~**OP-27 (parallel implementation) — Using Aequitas while the world still uses money.**~~ **Closed 2026-08-23 (§5.5), ruled and stress-tested.** Both directions across the boundary are deliberately costly; a money-made good is dark until sold in and clears via a **published template**; selling out keeps the debit and reads as a **gift**, because money is not matter (A1). **No axiom moved and no cross-boundary rule was needed.** Extraction self-limits through the extractor's own ratio gate, and **money cannot buy standing at any scale** — the workers are credited the hours, not whoever paid them. **Two items were registered rather than solved, and one is now closed.** **Repeat-shell entities — closed in v0.22 by §3.2c**, after two outside economists showed the gap sat on the main adoption boundary rather than at its edge; an organisation's debit is its members' debit, so closing one moves nothing. **Template capture** stays open (above, with OP-24). Paper: `00-strategy/OP-27_parallel_implementation.md`.
- ✅ ~~**OP-23 (shared overhead) — Shared-overhead attribution.**~~ **Closed in v0.5** (§6.2b): capital and overhead accrue to the asset and its holders and **never allocate to co-products**, so there is nothing to attribute. The interim inherited-proportions convention was deleted, not refined. `00-strategy/OP-23_capital_and_pollution.md`. **Re-attacked and held, v0.21** — an outside economist review argued the exclusion contradicts A5. **The ruling stands and A5's wording was repaired instead** (A5 above, the §6.2b box, register **B8**); the reply — *capital discipline runs through the builder's own gate, not the price tag, exactly as §7.2 argues for pollution* — is now written down rather than assumed.
...
Which activities are *always* creditable — childcare, schooling (and whose schooling), subsistence farming, untrained medical assistance — is **not** settled by this document, and should not be. It is exactly the kind of question A8 (no governing body) leaves to open variance, competing in public.
