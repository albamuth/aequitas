...
| 🔴 | **OP-24 (understatement drift) — Understatement drift** | Errors favouring subscribers have no funder. Attacks A4 (no externalities). **Lever enlarged in v0.6** by the stock/baseline constants. **Partially relieved in v0.17 (OP-26): the audit of *extent* has a funder even though the audit of *weight* does not.** And §5.3b **dissolves the *funding* half entirely** — "funding" in Aequitas is only the recognition of an activity as creditable, recording is never gated (A7), so the credit for audit work was never scarce. **What OP-24 is actually about is the surviving half: demand.** Someone must still *want* the correction, and for an understating weight constant the rival sector remains the only party who does. The objection is narrower and sharper than it was, not closed. |
