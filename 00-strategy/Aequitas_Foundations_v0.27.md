...
> **Version:** 0.27
> **Date:** 2026-08-27
> **Status:** Working foundations. **Truncated at §8** — the OFCS comparison is gone, and the conformance list, the open problems and the adoption reading now live in their own documents (§8 below). **§5.2–§5.5 are the owed remainder of the §5 rewrite.**
> **This version:** **every kind of work a network credits has a published evidence rule** — §6.4b gains it, and §6.6 says plainly that conservative weighting works inside a rule and never in place of one. Answers a drafting-council question of 2026-08-22 about who consents to IC-8's denominator.
> **Previous version (v0.26):** **the floor rule no longer passes through a subtraction.** §5.1a gains the general rule and a third landing state, **`not identified`**; §5.1b's *"report the residual as a lower bound"* is replaced by per-operand interval propagation; §5.1c's coverage percentage carries the same repair. **Found from outside** — @cairn-lineage, c23607 on 1f916.ai #2259, conceded in public at c25746.
...
> **Companion:** `00-strategy/Aequitas_EventLog_v0.10.md` — the C1 data model, which carries the §5.1b scope-alignment check as §7.2a.
...
>
> **⚠️ What was withdrawn.** Earlier versions said *"because self-care is credited work (§6.1b), the credited-labour pool is ~3.4× all productive labour"*, and Q1 published a labour row of **3,647 h/yr available against 1,600 h/yr needed, ratio 2.28.** **That row cannot fail, so it was never evidence.** Its numerator is **credited** hours, which include the self-care floor — and `F` is a value the network sets by rule (§7.5.1). **The pass condition is fixed the moment `F` is chosen, before a single worker is counted.** Sleeping is credited work under §6.1b and it cannot lay cable.
>
...
##### The floor rule applies to a count. It does not survive a subtraction
...
**Read the rule above again and note what it rests on: under-recording can only understate a total.** That is true of a **count**. It stops being true the moment the incomplete figure is **subtracted from** something, because a subtrahend reverses the direction of every error inside it.

**So the rule holds for `A`, and reverses for `−A`.**

###### An example, with the numbers

Take `R = N − Y`, where `N` is an independently known total and `Y` is what measured producers recorded (§5.1b). Published figures: `N` = 88,000 t, `Y` = 82,000 t, so `R` = **6,000 t**.

| Which operand is blind | What is really true | True `R` | Published `R` | So 6,000 is |
|---|---|---|---|---|
| **`Y` under-records** by 4,000 t | `Y` = 86,000 t | **2,000 t** | 6,000 t | an **upper** bound — 3× the truth |
| **`N` under-observes** by 10,000 t | `N` = 98,000 t | **16,000 t** | 6,000 t | a **lower** bound |

**Same figure, same incompleteness, opposite labels.** Calling 6,000 t a floor in the first row overstates the dark pool three-fold.

> **The direction of the error decides the label, and the direction is a fact about which operand is blind.** *"Some coverage is incomplete"* does not tell you which.

**How to label a derived figure.** Propagate the bound on each operand and read the interval:

> **`R ∈ [N_L − Y_U, N_U − Y_L]`**, where `_L` and `_U` are the low and high bounds on each operand.

- **`floor`** — only when the interval is bounded below by the published figure, which needs a stated directional argument about **each** operand's blind spot.
- **`ceiling`** — the mirror case.
- **`not identified`** — **the default.** Where either blind spot has no defensible direction, the figure is published with the interval and neither label.

**`not identified` is a third landing state, and it is where a derived figure starts.** A label is promoted onto it by an argument, never assumed from the shape of the inputs. **This is §5.1b's problem in its general form**, and it reaches every figure built by combining two incomplete readings — the residual (§5.1b), the coverage percentage (§5.1c), and any future rule of the same shape.

> **⚠️ Found from outside, 2026-08-27.** @cairn-lineage, comment c23607 on 1f916.ai post #2259, conceded in public at c25746. **§5.1b carried the unqualified sentence for six versions**, and it was produced by carrying this section's floor rule through a subtraction. **The error ran in the project's own favour** — an overstated residual makes the dark pool look larger, which is the direction the adverse-selection argument (§5.1b) and the deliberate under-count of `Z` both want. **A wrong number that flatters the theory is the class least likely to be caught from inside.**
...
**What to publish when the check fails.** A mismatch downgrades the claim rather than invalidating it, exactly as the floor rule downgrades a partial count. **Publish the interval and the boundary and window actually observed.** A residual is attributed to a named person only on attribution evidence, never on membership of a cohort (§5.1c).

> ### 📦 WHICH LABEL THE RESIDUAL GETS, AND WHY IT IS NOT ALWAYS A FLOOR
>
> **`R = N − Y` is a subtraction, and the floor rule does not survive a subtraction** (§5.1a). **The label depends on which operand is blind.**
>
> | Which operand is blind | Effect on `R` | The correct label |
> |---|---|---|
...
> #### An example, with the numbers
>
> The valley wheat case from the box above, after the four rows are fixed: `N` = 88,000 t, `Y` = 82,000 t, `R` = **6,000 t**.
>
...
>
> **Now do it properly.** Suppose the satellite is known to under-detect but by an unquantified amount, and the registry is audited and sound: `N ∈ [88,000, 100,000]`, `Y` = 82,000 ± 0.
>
...
>
> **⚠️ What this replaces.** Through v0.25 this section said *"report the residual as a lower bound"*, unqualified. **That was wrong in the `Y`-blind case and it was our own sentence, not a critic's construction.** Conceded in public 2026-08-27 (c25746) to @cairn-lineage, c23607 on post #2259.
...
> **⚠️ The coverage figure is `Y ÷ N`, so it carries §5.1a's subtraction problem in its own form.** It is built from the same two incomplete readings as the residual, and **which operand is blind decides which way it is wrong.**
>
...
>
> **So a coverage percentage is labelled by the same three states as the residual** — `floor`, `ceiling`, or **`not identified`** — and **`not identified` is the default** until there is a stated directional argument about each operand (§5.1a, §5.1b). **A network publishing a bare percentage with no direction on it is publishing a number nobody can use.**
...
- **Weak evidence weighs near zero until corroborated** (§6.6). Unwitnessed "I thought for ten hours" is *recorded* faithfully (A7) but carries the weakest basis and lowest confidence, so conservative weighting values it at ≈0 until something corroborates it. Fabrication is cheap to assert and cheap to hold — an incentive, not an enforcement rule. **This works inside a rule, never in place of one.** A published rule may accept evidence of differing strength, and the weaker basis weighs less. **A claim with no matching rule is a different thing: it is recorded, and it credits nothing.** See the next subsection.
...
#### Every kind of work a network credits has a published evidence rule

> **A trust network publishes, for every kind of work it credits, what evidence that work requires. The published set is its contract with the subscriber.**
...
2. **A network should not write a rule for work it cannot get evidence about.** Where it cannot say what would count as evidence, it does not credit that kind of work, and it says so in public.

**So "work the network credits but measures badly" is not a case this design produces.** A kind of work is either covered by a published rule, and the evidence exists and the hours credit in full — or it is not covered, and nobody was promised credit for it.

##### An example, with the numbers

A network publishes its rule for translation work: **the translated text exists, and the client confirms receiving it.**

| | Hours worked | Against the published rule | Hours credited | Added to the lifetime pledging budget |
|---|---|---|---|---|
| **A** delivers a translation, the client confirms | 20 h | Rule met | **20 h** | **20 h** |
| **B** says they translated for 20 hours, with no text and no client | 20 h | Rule not met | **0 h** | **0 h** |

**B is not being judged, and B is not being punished.** B knew the rule before starting, because the network published it.

**Now change the network instead of the person.** Suppose translating for someone who cannot confirm — a dying relative, a stranger in a queue — is work this network wants to credit. **Then it writes a rule for that case**, perhaps a second witness, perhaps a recording. **Once the rule exists, B's 20 hours credit at 20 hours.** They do not credit at 0.4 hours because somebody weighted a vague claim cautiously.

##### What this answers

**The question it answers is: a person whose work a network measures badly earns little credit, and therefore little pledging power under IC-8. Who agreed to that denominator?**

> **The subscriber did, when they subscribed, because the rules were published first.**

**Three things bound what that can cost anybody.**

| | |
|---|---|
| **Subsistence is never at risk** | Self-care is verified by proof of life (the table above), which every living human meets at near-zero burden. **Nobody's floor depends on documenting anything.** |
| **A network with bad rules loses people** | Sellers choose which networks they accept (§5.0), and a counterparty discounts credit it cannot check. |
| **Nobody is enclosed** | Not participating is always available, and always was (§5.3c). |
...
**This does not make every kind of real work documentable.** Some kinds will not be, and a network that cannot write a rule for one leaves it uncredited.

**What changes is where the gap sits.** It moves from a weighting nobody can see into a published list anybody can read before joining. **A person can tell in advance whether their work counts, and can choose a different network, or none.**

> **§6.5a already states the other half of this**: *"the accounting covers what is claimed and attested. Everything else is life."* **That says the system does not need to capture everything. This says a network must state in advance what it does capture.**

*(Author ruling, 2026-08-27, answering a drafting-council question of 2026-08-22: **IC-7 and IC-8 encode a shape of a life — who consents to that denominator?**)*
...
2. **Conservative weighting of low-confidence flows** (C1 (event-log schema) §12) does the real work. Self-asserted, unwitnessed, near-zero-material work carries the weakest `basis` and lowest `confidence` in the log. Weighed at the **pessimistic end of its interval — which for a credit claim is close to zero** — the fabricated hours are recorded faithfully and are worth almost nothing until something corroborates them. **This is the general answer to unobservable work, and it is an incentive rather than an enforcement rule: vagueness is cheap to assert and cheap to hold.**
   ...
   > **Read this alongside §6.4b's published-rule requirement, added in v0.27, because the two are often confused.** Conservative weighting decides **how much a claim weighs inside a rule the network has already published.** It never stands in for a rule that was never written. **A claim that matches no published rule is recorded and credits nothing** — not *"almost nothing"*, and not as a judgement about the person. **A network that finds itself weighting a whole class of real work near zero has written the wrong rule, or has failed to write one.**
