...
- [C. Tests not yet run](#c-tests-not-yet-run)
...
| | Item | Status |
|---|---|---|
| 🟠 | **OP-28 (residual denominator) — What the leftover is divided by** | **Registered 2026-08-29. The producer side is answered and folded on 2026-09-02. The extent register is the surviving open half.** §4.4 used to divide the leftover by `Z`, **a count of unmeasured producers** — but §2.5 requires a measurement wherever the divided thing left a physical trace, and **land left one, while a head-count did not.** **Author ruling, 2026-09-02 (S3): the division is withdrawn.** `06-simulation/producer-side-splitting/` (run 2026-08-31, 11 self-tests) put **two worlds with truths differing by 1.21× in front of one network and handed it identical books** — same `N`, `Y`, `Z`, `R` — so **no rule computed from those books separates them**, and the two rules that get the number right (S1, S2) are the two a network cannot run from one book. **Folded to Foundations v0.37 §4.4:** `R = N − Y` is computed, **published whole and divided by nothing**; **A7's per-producer estimate comes from §4.1's production model**, still over the leftover rather than the population; **the rule telling a network to under-count `Z` went with the division it fed**; Conformance **v0.11** row 14 rewritten and row **14d** added; Overview **v0.24** carries the reader-facing half. **⚠️ What stays open, and it is the whole of what is left:** a production model distributed by **measured extent** is itself unaudited. **Honest noise barely moves it (0.96× at 50% noise). A producer declaring half their real extent returns it to 1.33×, exactly where the withdrawn rule sat.** The claim that the survey behind `N` catches that is **an argument nobody has tested** — and the survey-check model that would test it has not been built. **Until it runs, a network publishes its extent register as unaudited (conformance 14d).** Paper: `OP-28_dark_production_and_the_residual_denominator.md`. |
...
| 🟠 | **OP-16 (onerousness gap) — Onerousness gap** | **Both halves now reachable, neither measured.** The contingent reserve is **withdrawn** (2026-09-18); a pledge names a person or the criteria that will name one, and committed room is consumable. **So society can simply pledge more for a job nobody wants, dull or dangerous alike** — where the reserve only ever reached hazard, because only hazard leaves a causal tail. Skill half fixed by A2 v0.3. **A2 is untouched: credit is still one hour per hour, and it is demand deciding rather than a rate.** **Whether it actually staffs dull work is unmeasured.** |
| 🟠 | **OP-6 (feedback mechanics) — Feedback mechanics** | Promoted; also carries signal flooding. **Sharper after 2026-09-18**: committed pledged room is consumable, so *whoever is already liked attracts the most pledges* is now a route to material consumption and not only to influence (§5.5.5). **The bound is that every pledge costs a real person a real hour from a finite budget that cannot be manufactured.** |
...
| `24 ÷ F` reported as a bound on **consumption** | A bound on credit reads as a bound on what people get | **It bounds pledge budgets, which are earned credit.** Consumption is gated by `D ≤ ρ·(C + P)` and `P` has no per-account limit — **Foundations §5.5.5** |
...
*Foundations adopts this:* §3.4 is narrowed, §3.4a states the material/energy rule, §4.5 removes the overhead allocation, and §2.5 has now lost two rows rather than gaining entries.
...
**Hours-worked is an allocation convention, not a measurement.** Foundations §2.5 declares it as one, which converts a universality violation into an honest one. **It does not make the convention correct**, and a critic can still ask why hours rather than some other basis.
...
> **Auditing cost constants is one of the problems a trust network exists to solve.** How it does so is the network's own design (§2.6 — *state what must be true, never how to build it*), published and checkable like everything else it does.
...
**⚠️ Still unproven, and now honestly labelled as such.** No network has demonstrated a working answer, because no network exists. **The five requirements are what a design will be judged against, not evidence the problem is solved.** The test that would settle it has not been run (§C item 2): simulate a population of trust networks under these incentives and find the conditions under which drift stops being arrested.
...
**Attacks:** the "surgical, keep functional institutions" positioning in §2.6; constrains OP-1 (service → influence).
...
**What survives.** The coordinator-class risk was never *only* about income. It is about **who holds empowering work**, and **§2.6 deliberately keeps existing institutions** — municipal government, planning bodies, civil service — precisely the institutions in which conceptual and agenda-setting work is concentrated. Removing profit does not redistribute *that*.
...
**Status:** 🟢 **Written up (v0.10). Standing statement: `00-strategy/open-problems/OP-9_calculation_reply.md`.** Plain-language version in Overview §6. Answered for cost; one residue terminates in OP-10 (below).
...
>
> **Ruling: OP-22 is NOT split. OP-22b is NOT opened.** Full paper, with the arithmetic: [`OP-22_identity_not_disclosure.md`](open-problems/OP-22_identity_not_disclosure.md).
>
...
**The cheap structural answer, adopted in Foundations §3.3a and §2.3:**
...
>
> **Three notes kept for honesty.** (1) This is a **declared convention, not a measurement** (§2.5) — hours leave no trace pointing at a particular debit; hours are chosen because they add no new lever. (2) It **does not touch §4.5**, where an asset's creation-cost splits by *holding time* so a new hire bears about zero — that is property debit on an asset, this is consumption and operating debit. (3) It **may also close §3.4a's open residue** (apportioning a jointly-caused debit across a team), which has the same shape and now has the same answer. **Not claimed until checked.**
>
...
<!-- tag: obj-b7 -->
## B7 — OP-17. Joint production ✅ **CLOSED — reopened and re-closed 2026-09-03 by removing the division**

> ### 🔄 RE-CLOSED 2026-09-03 ON A DIFFERENT MECHANISM. READ THIS FIRST.
>
> **The split rule this entry closed OP-17 with is withdrawn.** `06-simulation/method-spread/` swept **36 methods that all satisfied all four of Foundations §3.4a's published obligations**, on real DOE refinery process energies, and found them **6.31× apart** on LPG. **The layer §3.4a called a declared convention moved the answer 1.29×; the layer it called a measurement moved it 6.31×.**
>
> **The narrowing below anticipated exactly this and named the test. The measurement came back wide.**
>
> **The repair is not a better split. It is no split.** *(Author ruling, 2026-09-03.)*
>
...
>
> **What that removes:** the declared basis, the routing model, the sub-process boundary choice, the four obligations, and the labour convention OP-18(α) closed. **What it needs instead:** which steps a product passed through — **binary, off a flow sheet, never a share.**
>
> **Why the books do not inflate: A3.** A debit is a **unique record of one specific event**, so two co-products naming the same 100 MJ name one record. **A ledger walk is a union over identified parcels, never a sum** — new conformance row 10e. Measured: across 128 readings of one refinery the union returns the chain total every time and a naive sum returns **111 different answers**, overstating by up to **7.00×**.
>
> **Yield leaves the ban and demand does not.** Output mass is physical and a scale reads it; demand and desirability are not and nobody can weigh them. **The tenderloin passage is rewritten: a smaller output does read dearer per kilogram, and who receives a scarce cut stays a distribution question.**
>
> **New in the bargain:** a coarse reading is a **ceiling** on a fine one, confirmed on **1,152 product-resolution pairs**. **So a producer wanting a lower figure has to buy more measurement**, and resolution stops being a lever to police. `06-simulation/chain-resolution/RESULTS.md`.
>
...
>
> > **Aequitas fixes the obligations on the split. It does not fix the method, and it cannot — no single model fits every industrial process.** Determining how a process divides its inputs takes expertise in that process. **The method belongs to the industry, under §2.6's scope rule: state what must be true, never how to build it.**
>
<!-- struck-ok: the register records the wording that was struck -->
...
>
> **Registered, not solved:** nobody has measured **how far a split actually moves** across honest methods. The test has not been run — §C. **If the range is narrow the obligations suffice; if it is wide, method choice belongs with OP-10.**
>
...
| **A §2.5 row was deleted rather than filled in** | The strongest possible outcome for a declared-convention entry. |
...
| **3 — Risk-dumper (mislabeling)** | An employer labels speculative work "pledged" to recruit, or vice-versa to shed risk | **The label is read off the *public* pledge ledger, not declared** (§4.7). Same move as co-product splits (§3.4a) and cost constants (§3.3a): never let a self-interested party write the number. |
...
**PASSES.** No axiom moved, no mechanism changed, and the arithmetic is unaffected. **It is a recording rule, and its real gain is that it makes §3.2b's use-side half observable** — a purchase is an event; a combustion is not. **Two wording items are still unwritten and both are named above.**
...
**P9 (local-currency read) — the rollout plan read as a local currency. ✅ Fixed in Foundations §5.6.** **Aequitas's overlay computes a number money cannot produce.**
...
5a. ~~**Split-robustness test — how far does a split move across honest methods?**~~ ✅ **RUN 2026-09-02, and it retired the rule it was testing.** `06-simulation/method-spread/` swept **36 honest methods, every one satisfying all four obligations**, on real DOE refinery energies. **WIDE: five of seven fractions crossed the declared 2.85× threshold, and LPG moved 6.31×.** The lever was the **modelled conversion routing**, not the declared convention, which moved only 1.29×. **Rather than route method choice to OP-10 as this item proposed, the author withdrew the division entirely on 2026-09-03** (B7). **Legs 2 and 3 were never built and are no longer needed**, because there is no longer a split to sweep. **Replaced by `06-simulation/chain-resolution/`**, which measures the rule that took its place.
