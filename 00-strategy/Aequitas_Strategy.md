...
> - **The conformance list.** The things that must be true for an implementation to *be* Aequitas — conservation holds, the ledger is derived and never stored, credit never transfers, coverage is published with its extent, and so on. **[`Aequitas_Conformance.md`](Aequitas_Conformance.md) carries the current list**, moved out of `Aequitas_Conformance.md` on 2026-08-25.
...
- **C1 (event-log schema): Event log schema.** ❌ **Retired 2026-08-28.** The record model was never asked for and it read as a specification, which contradicts §2.6 and the simulator ruling of 2026-08-23. **The arithmetic it carried — IC-1 to IC-12 — is now in `Aequitas_Conformance.md`.** A trust network writes its own schema.
...
- [ ] `Aequitas_System_v1.0.md` — the system stated and tested, plus the conformance list ([`Aequitas_Conformance.md`](Aequitas_Conformance.md)). *Renamed from `Aequitas_Protocol_v1.0.md` on 2026-08-23: "protocol" named the deliverable after the smaller half of it.*
...
- [ ] Every open problem in `Aequitas_Objections.md` either solved, dissolved, or explicitly scoped as v2 — **the remaining live blockers are governance: OP-10, OP-24, and OP-16's tedium/indignity half (its hazard half is addressed by the contingent reserve, §4.6).**
...
2. **Conformance requirements** — [`Aequitas_Conformance.md`](Aequitas_Conformance.md), precise enough to check an implementation against. **Audience: implementers.** Not a schema, not a protocol, not a product.
...
> **Moved here on 2026-08-25 by author ruling.** This was Foundations v0.23 §11 and the tail of `Aequitas_Conformance.md`. **Neither states what the system is, so neither belonged in Foundations.** Nothing was cut.
