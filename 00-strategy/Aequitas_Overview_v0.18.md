...
> **Rigorous version:** `00-strategy/Aequitas_Foundations_v0.25.md`. This document is the plain-language companion; where they differ, the Foundations govern.
...
>
> **Here is the case that shows why, with the numbers.** One person works 8 hours on a Monday. Network A counts 4 hours a day as the work of staying alive, so it records **12** credited hours. Network B counts 10, so it records **18**. **Both figures are right**, because each network read the same two facts — eight hours worked, one human alive — through its own settings. **Add them and you get 30 hours in a 24-hour day, which the system forbids. Nobody adds them.** No network holds that sum and no purchase is ever checked against it.
>
> **And a person holding accounts with both can reach more than a person holding one.** Against someone who only stays alive, the two-account person reaches **36.0 hours of room against 12.0 — three times**, where Network B's own books state a limit of 2.4 times. **No book is wrong.** Whether this stays small in practice has not been measured, and this project says so rather than assuming it.
>
...
>
> Reaching 2.4 needs 24 credited hours every single day, birth to death, for eighty years. Here are four whole lives in a community with a 10-hour baseline:
>
...
>
> **The figure to quote is about 1.6 times, not 2.4.** A very hard working life reaches 1.6 times a life spent only staying alive. **2.4 is the wall nobody gets to.**

**Four honest conditions, because this is the theory's single biggest claim and it would be easy to overstate.**
...
| **That it is one network's books** | **Not a statement about anything wider.** See the box in §0. A person holding accounts with two networks reaches three times a floor-only person, where one network states 2.4. |
...
| Aequitas | Walled at **2.4 times** inside one network's books, and a very hard working life reaches about **1.6 times** — and everyone alive holds some, because staying alive earns credit |
...
**Who checks the cost estimates themselves? This is the weakest spot in the whole system, and it is worth stating plainly rather than glossing.**
...
**And the honest part: nobody has built one of these yet, so nobody has shown that it works.** Those five are the test a design has to pass. They are not proof it passed.
...
*For the rigorous statement of everything above, with the arguments and the open problems in full: `00-strategy/Aequitas_Foundations_v0.25.md`.*
