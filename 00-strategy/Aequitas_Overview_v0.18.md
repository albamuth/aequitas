...
> **Rigorous version:** `00-strategy/Aequitas_Foundations_v0.28.md`. This document is the plain-language companion; where they differ, the Foundations govern.
...
>
> **And you cannot add the two figures to work out what the person can "really" reach.** It is tempting and it is not allowed. **An hour in Network A's books and an hour in Network B's books are not the same unit.** A debit is a bundle of physical quantities — kilograms, energy, hours, land — and it becomes a single number only when a network squashes that bundle down using **its own** weighting. **A and B weigh things differently, so the same basket of goods comes out as a different number in each.** Adding them would set one A-hour equal to one B-hour, which is an exchange rate — **and an exchange rate between two sets of books is a currency, which is the one thing this system does not have** (§3, and Foundations A3).
>
...
*For the rigorous statement of everything above, with the arguments and the open problems in full: `00-strategy/Aequitas_Foundations_v0.28.md`.*
