...
| 2c | **A business is never the final holder of a debit. Only people are.** Whatever its workers caused **while acting for it** is at every moment their own debit, divided by the hours each worked there. **A worker's own errand is theirs alone.** **Closing the business moves nothing and clears nothing.** | A1, §3.2, §3.2c |
| 2d | **A transport emission is recorded against whoever held the fuel parcel and burned it, and never against the goods that journey carried.** An implementation must not let a holder discharge a burned-fuel debit by tagging it onto anything it hands off. **Handing on the *unburned* parcel is the only transfer that moves it.** | A1, A5, §3.2b, §3.4a |
...
| 4b | **What a subscriber enters into the accounting is auditable by the network that keeps it.** Entering a claim is what makes it checkable, and a network cannot verify arithmetic it may not read. **What a subscriber keeps out of the accounting was never in it** — §4.4's five legitimate cases stand, and produce not entered cannot be sold on the network. **This is disclosure to the network, and it is not independence:** a record made on one path stays a self-check however openly it is published. | §4.2, §4.4, §4.3, §4.7 |
...
| 6a | **The network publishes a procedure for correcting records** — for a **mistake** (the quantity is wrong), **bad data** (the instrument, method or source was faulty), and **fraud** (the event did not happen). **Four properties.** It is **published and readable before a person joins**; it says **who may raise a correction and on what evidence**; it says **what happens to the record while the correction is open**; and every correction is **dated, attributed and appended**, per row 6. **What the procedure is, is the network's design. That one exists and is published is not optional.** | §4.4, §4.2, §2.6 |
...
| 10h | **A passage claim is published by the producer who makes it, and reconciles against records made on a different path** — metered energy, intake weights, output weights — **within a stated margin.** A passage says which steps a co-product went through, and **a false one lowers that product's figure by up to 6.25× on §3.4a's own worked spread.** A claim that reconciles against nothing but the producer's own flow sheet is a self-check and **is reported as unreconciled, never accepted as established.** | §3.4a, §4.3, §4.4 · row 7 |
...
| 14e | **A joining person's declared span is a record.** Where it is unevidenced the network sets it by one published rule, **back to the earliest date the evidence allows**, and what that rule produces is a record — corrected by supersession under rows 6 and 6a, **and carrying no interval and no label.** A **declared span** is how long the person says they have lived, and it sets how many years the reconstruction rebuilds. **It is not exempt from the rule that an assertion is not evidence.** The rule names a date and not a direction, because each declared year opens `ρ·F·365` hours of room while adding about a year of consumption: **above `ρ·F` = 3.78 h/day a longer span pays the filer, and below it a shorter one does.** Both settings are inside the workable band. **Row 13's three labels do not reach it**, because they exist to repair a subtraction's reversal of direction and the earliest-date rule is not a subtraction — **the earliest permitted date gives the longest permitted span, so the figure is one-sided by construction.** | §4.4, §5.5.3 |
