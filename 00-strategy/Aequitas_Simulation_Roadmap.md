...
> **Companions:** `Aequitas_Foundations.md` §2.6/§6 (scope, where the rest of the project lives), `Aequitas_Strategy.md` §2, `06-simulation/scenario-suite/scenario_suite_METHOD.md`.
...
[`Aequitas_Conformance.md`](../00-strategy/Aequitas_Conformance.md) carries **16 requirements that must hold for an implementation to be Aequitas.** They were written for an implementer. **They are also exactly what the kernel should assert, every period, in every scenario.**
