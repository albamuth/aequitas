...
> **Depends on:** `Aequitas_Foundations_v0.26.md` A1–A8
...
> | Compare | Holds when |
> |---|---|
> | **Quantity** | Both figures count the same measured thing, in the same unit |
> | **Boundary** | The two `extent` values describe the same domain |
> | **Window** | The two `as_of` values cover the same period |
> | **Resolution** | `uncertainty(N) + uncertainty(Y)` is **smaller than** `N − Y` |

**When any row fails, the residual is not refused — it is downgraded.** It is reported as a bound with the mismatch named, exactly as §7.4's extent rule reports a verdict whose closure basis is absent, and as §8.1's floor rule reports a count over incomplete coverage. **A record is never blocked for being imperfect; it is published with what is wrong with it stated.**

**Worked case.** `N` = 100,000 t at `extent: valley/whole`, `as_of: 2026-crop-year`, `uncertainty: ±12%`. `Y` = 82,000 t at `extent: valley/upper`, `as_of: 2026-calendar`. Rows 2 and 3 fail outright, and row 4 fails too: `±12,000` is not smaller than `18,000` once `Y`'s own bound is added. **The correct output is not `R = 18,000`.** It is *"residual not computable at this alignment"*, with the three mismatches listed, which is a result a counterparty can act on and a bare 18,000 is not.

> **Why this belongs beside IC-12 rather than as IC-13.** It is not a constraint over recorded quantities — it is a **precondition on an operation**, of the same kind as the resolution and monotonicity rules in §8. Shipping it as a tenth integrity constraint would repeat the §12.3a mistake, where **IC-13** and **IC-14** were rejected precisely for checking a self-asserted field rather than one recorded quantity against another. **This checks two records' declared scopes against each other, which is what the provenance block was built to make possible.**
