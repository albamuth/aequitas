...
| | Item | Status |
|---|---|---|
| 🟠 | **OP-28 (residual denominator) — What the leftover is divided by** | **Registered 2026-08-29. Partly folded the same day; the denominator itself is open.** §4.4 divides an unattributed leftover by `Z`, **a count of unmeasured producers** — but §2.5 requires a measurement wherever the divided thing left a physical trace, and **land left one.** A producer holding an account with two networks is absent from `Z` while half their output sits in the leftover, so **the leftover is divided among producers who did not make it, and it does not converge**: onboard everyone else and the arithmetic reaches `50 t ÷ 0`, with coverage stuck below 100% and no way to say whose. **The candidate repair is a measured extent — unrecorded hectares, vessel-days — falling back to a headcount where nothing was traced.** It is computable from one network's own books, and **reading another network's records was never forbidden** (§4.2, *comparison never conversion*). **Both cheat directions are already constrained**: under-declaring your extent dangles against the same survey that produces `N`, and over-declaring makes your goods dearer with no rule needed. **Unmeasured. Owed: `sr-20260829-producer-side-version-of-the-cross-network-s`.** Three findings from the same reading WERE folded — Foundations **v0.34** stops describing non-participants as *charged*, separates **unsubscribed** (a person outside) from **unrecorded** (output not in the books), and names five legitimate cases of unrecorded output; Overview **v0.21** carries the reader-facing half. Paper: `OP-28_dark_production_and_the_residual_denominator.md`. |
