...
For Aequitas that collides head-on with two core rules: that a price is just a cost with no markup ([Foundations §A5](../00-strategy/Aequitas_Foundations_v0.13.md#a5-price--cost)), and that cost is measured as physical flow, not money ([Foundations §A1](../00-strategy/Aequitas_Foundations_v0.13.md#a1-materialism-of-cost)). **We'd be using price to compute the very thing we say price should be replaced by.** An economist will spot this instantly, and if we haven't addressed it up front, it discredits the number.
