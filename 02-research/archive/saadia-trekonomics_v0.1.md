# Trekonomics: The Economics of Star Trek

**Type:** book (popular economics)
**Author(s):** Manu Saadia
**Published:** 31 May 2016, Pipertext / Inkshares, 280 pp. ISBN 9781941758755
**Retrieved:** 2026-07-31
**URL:** https://en.wikipedia.org/wiki/Trekonomics
**Related:** Rick Webb, *The Economics of Star Trek: The Proto-Post-Scarcity Economy* (5th anniv. ed. 2019, foreword by Saadia)
**Local copy:** *commercial book — buy, don't download*

## Why this matters to Aequitas

Not theory — **legibility**. It is the most widely read popular treatment of a moneyless economy, and it gets a general audience past the "but what would motivate anyone?" objection faster than any argument. Bears on [[enrichment]] / OP-6 and on the public-facing text (doc 4 in the roadmap), **not** on the implementer spec.

## Key findings

- Core thesis, and the most quotable line available: **"Post-scarcity is not so much a matter of material wealth or natural bounty, but an organizational option for society."**
- The Federation has **no money** — therefore no salaries, no revenue, no profit, and most of the machinery of capitalism simply absent.
- Motivation is **reputation and respect**. People work harder than we do, pursuing science, teaching, exploration, and self-cultivation.
- The **replicator** functions as device and as metaphor for automation, and is treated as a **public good** — universally accessible, not owned.
- Documented critique: the book **underweights psychological barriers**, specifically social identity theory — in-group favoritism and cognitive bias could undermine a post-scarcity society regardless of material abundance.
- Saadia has had unusually broad mainstream pickup (NYT, WaPo, FT, WSJ) for a book of this kind.

## What we can use

- **The "organizational option" framing is the single most useful borrowed sentence for public writing.** It is Aequitas's own claim: the binding constraint is not productive capacity but how we account for it. Aequitas exercises that option now, without replicators.
- **Reputation-as-motivation is [[enrichment]] in fictional form.** Where readers find Saadia's version implausible is free user-testing for OP-6 — the popularity-plutocracy problem.
- **Public-good infrastructure** matches the [[no-taxation]] story on infrastructure provision.
- The social-identity critique is a **direct warning about OP-6**: removing material scarcity does not remove status competition, and a system where enrichment *is* status may intensify it.

## What it gets wrong / limitations

- **No mechanism whatsoever.** It describes a destination with no route.
- **It assumes away the entire problem.** With replicators and effectively unlimited energy, allocation is trivial because nothing is scarce. Aequitas's whole subject is allocation *under* scarcity. Saadia's world needs no [[debit-tolerance]], no [[price-equals-cost]], no [[verification-ladder]] — it needs no accounting at all.
- It is **exegesis of a television programme**, not economics. Leaning on it for feasibility is a serious credibility hit with exactly the audiences Aequitas needs most.

## Rule for citation

**Use Saadia to make the goal vivid. Never to argue that the mechanism works.** In the technologist and academic papers, cite at most in passing or not at all. In the public-facing book, it is genuinely valuable as an on-ramp.

## To do

- [ ] Buy and read properly — the above is from secondary sources
- [ ] Check Rick Webb's *Economics of Star Trek*: he attempts an actual mechanism (a reputation/UBI hybrid), which may be more useful than Saadia for OP-6

## Related

- [[post-scarcity]] · [[enrichment]] · [[calculation-in-kind]]
