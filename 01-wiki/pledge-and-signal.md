...
- **To the receiver it is virtual credit.** It expands the receiving person's or co-op's [[debit-tolerance|debit-room]] so a fixed cost bites less. The debit itself does not move to the pledger and does not vanish — it stays where it was incurred (e.g. holding-time-split on a facility's holders, [[debit-taxonomy]] §4.5); the pledge only *cushions* the bite.
