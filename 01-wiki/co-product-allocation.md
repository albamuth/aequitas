...
**Fate sets ledger character; the process sets cost share.** Manure is pollution debit in a lagoon, a genuine co-product in a biodigester, and an *observed* fertiliser offset when spread on fields. [event-record](event-record.md)'s fate closure already records this — no new machinery.
...
**Labour.** The farmer's eight hours were spent on the animal, not on the hide. Nothing in physics apportions them, and splitting them in proportion to metabolic energy would be an assumption wearing a measurement's clothes. **This is OP-18, and it now blocks the estimation engine** — see [physical-trace-test](physical-trace-test.md).
...
| **Constant capture** — publish energetics favourable to your sector | [rival-sector-audit](rival-sector-audit.md). **Open — OP-24.** |
...
- [event-record](event-record.md) — the split is computed at projection time; **no schema change was required**
- [retroactive-reweighting](retroactive-reweighting.md) — better process science re-splits historical production
...
- [rival-sector-audit](rival-sector-audit.md) — what disciplines the constants the split depends on
