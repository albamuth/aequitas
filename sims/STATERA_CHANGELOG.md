...
> Version-by-version change history for [`statera.py`](statera.py) and its results doc [`STATERA.md`](STATERA.md). Newest entry first. Read this only when tracing **when and why** something changed.
>
...
>
> The kernel is **step 1** of [`../docs/Aequitas_Simulation_Roadmap_v0.2.md`](../docs/Aequitas_Simulation_Roadmap_v0.2.md). It is an instrument for testing the theory. It is **not** a trust-network database, and it is **not** a first version of Aequitas — see Foundations §1.2 (the scope section, *"what Aequitas is, and what is therefore out of scope"*) in [`../docs/Aequitas_Foundations_v0.19.md`](../docs/Aequitas_Foundations_v0.19.md).
...
**Full paper: [`../docs/Shelf_life_and_custody_v0.1.md`](../docs/Shelf_life_and_custody_v0.1.md).**
...
- **The disparity ceiling at a 10-hour floor:** 2.4000× at every `rho` in [1, 3]. Spread across the sweep 8.9 × 10⁻¹⁶ — floating-point noise. Matches [`disparity_ceiling_sim.py`](disparity_ceiling_sim.py), N = 200,000.
- **The clearing rate:** `rho*` = 1.20, median gets 0.92×, 35% constrained, disparity ≤ 2.40×. Matches [`rho_sweep.py`](rho_sweep.py), US production method.
...
**The absolute figures inherit their calibration.** `rho*` depends on OP-10 (weighting governance) and is illustrative, exactly as [`RHO_SWEEP.md`](RHO_SWEEP.md) says. What is claimed here is that **the kernel reproduces them**, not that they are settled.

---

*No axiom was contradicted in this version. Companion doc: [`STATERA.md`](STATERA.md).*
