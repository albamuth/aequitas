...
> **Tracks:** [`../docs/Aequitas_Foundations_v0.19.md`](../docs/Aequitas_Foundations_v0.19.md).
> **Run:** `python statera.py --test` · `python statera.py --demo`
> **Change history:** [`STATERA_CHANGELOG.md`](STATERA_CHANGELOG.md). No version notes appear in this document's body.
...
`collapse()` is that step. Foundations §3.2a (debit is a vector, collapsed on demand) is the source. **Whoever controls `w` controls every comparison in the system** — which is open problem OP-10 ([weighting governance](../docs/GLOSSARY.md#op-10)), the project's top blocker.
...
The credit rate is bounded below by the floor and above by the day, because IC-7 ([the 24-hour cap](../docs/GLOSSARY.md#ic-7)) caps any account at 24 hours of activity per 24 hours:
...
This is IC-1 ([mass balance](../docs/GLOSSARY.md#ic-1)) and IC-2 ([energy balance](../docs/GLOSSARY.md#ic-2)). A process id groups the events of one physical transformation. What went in must come out. A process id of −1 means the event is not part of a recorded process.
...
A pledge is a permanent grant of debit-room, backed one-for-one by lifetime earned credit. It is Foundations §6.4 (pledges and signals), and the constraint is IC-8 ([pledge backing](../docs/GLOSSARY.md#ic-8)).
...
A region's independently-measured total is 1,000 units, spread across 100 dark actors. From [`Onboarding_the_wealthy_v0.1.md`](../docs/Onboarding_the_wealthy_v0.1.md) §4:
...
**What is needed to build this:** a non-participant pool, a per-cohort join decision, a verification-cost dial, and generational time. Items 1–6 of [`Onboarding_the_wealthy_v0.1.md`](../docs/Onboarding_the_wealthy_v0.1.md) §8 name them all.
...
`κ_s` is the intensity for sector *s*: how much labour sits behind one dollar of spending there, directly and through the whole supply chain. Full method: [Labour and pollution intensity](../docs/GLOSSARY.md#src-labour-and-pollution-intensity).
...
The 1,380 h/yr median-lifestyle figure comes from [`MEDIAN_LIFESTYLE_RESULT.md`](MEDIAN_LIFESTYLE_RESULT.md); the $77,280 from the CE 2023 news release above.
...
**`statera.py` does not read an intensity table.** For the clearing reproduction it imports a single calibration constant from [`rho_sweep.py`](rho_sweep.py):
...
|---|---|
| [`STATERA.md`](STATERA.md): *"12 self-tests green"*, one period only, `kernel.py` | 22 self-tests, a full time axis with births and deaths, `statera.py` |
| [`STATERA_CHANGELOG.md`](STATERA_CHANGELOG.md): *"793 lines. 12 self-tests"* | **1,185 lines. 22 self-tests.** |
| [`STATERA.md`](STATERA.md) footer: *"Tracks Foundations v0.18"* | Header of the same file says v0.19 |

**Every link in the three Statera documents points at `kernel.py`, `KERNEL.md`, `KERNEL_CHANGELOG.md` and `KERNEL_PLAN_v0.2.md`.** None of those files exists. The rename to `statera.py` / `STATERA*.md` happened on 2026-08-23 and the internal links were not cascaded. [`Data_consumer-segmentation-archetypes_v0.1.md`](../research/Data_consumer-segmentation-archetypes_v0.1.md) carries the same stale path.

### 6. The low-floor narrative is quoted at one sample size

[`STATERA.md`](STATERA.md) and [`STATERA_CHANGELOG.md`](STATERA_CHANGELOG.md) both say *"reaching 12× needs somebody working a 22-hour day and the most anyone manages is 21.5."* That is the N = 50,000 draw, and it reproduces exactly. **At N = 200,000 the same code gives a top worker of 20.4 h/day and an observed spread of 10.22×.** The finding survives; the specific digits do not travel. Table in §8.

### Equations I could not verify against the code

| Relation | Status |
|---|---|
| `(N − Y)/Z`, the residual rule | **Not in `statera.py`.** Stated in §14 from Foundations §5.1b. Implemented separately in [`residual_unravelling.py`](residual_unravelling.py), which this paper did not audit. |
| Dollars-to-hours, `spend × intensity` | **Not in `statera.py`.** The kernel uses a single calibration constant (§15), not an intensity table. The method lives in [`median_lifestyle.py`](median_lifestyle.py) and its method note. |
| The `~40%` verification-cost stall threshold | Quoted in Foundations §5.3a from [`residual_unravelling.py`](residual_unravelling.py). Not reachable from Statera and not re-run here. |
...
- **`ρ*` = 1.20 is illustrative.** It depends on the weighting model, which is OP-10, exactly as [`RHO_SWEEP.md`](RHO_SWEEP.md) says. What Statera claims is that it reproduces the figure through different machinery.
...
- **The disparity bound is conditional, and Foundations says so.** §7.5 lists five conditions, of which the sharpest is OP-22 ([minimum audit disclosure](../docs/GLOSSARY.md#op-22)) — proving a claim is backed without exposing the history behind it. **Statera assumes that guard is implementable. It does not model it.**
...
|---|---|
| [`Aequitas_Foundations_v0.19.md`](../docs/Aequitas_Foundations_v0.19.md) | The theory. Axioms A1–A8 in §1; the debit vector in §3.2a; the transaction-time rule in §3.3; the residual rule in §5.1b; the self-care floor in §6.1b; pledges in §6.4; the basic-needs floor and the disparity bound in §7.5; the 17 conformance requirements in §9. |
| [`GLOSSARY.md`](../docs/GLOSSARY.md) | Every acronym, and the research source index. Link to its anchors rather than to research files directly. |
| [`Aequitas_Simulation_Roadmap_v0.2.md`](../docs/Aequitas_Simulation_Roadmap_v0.2.md) | The build order Statera implements steps 1–3 of. |
| [`Onboarding_the_wealthy_v0.1.md`](../docs/Onboarding_the_wealthy_v0.1.md) | The adoption arithmetic and the six things a run needs. |
...
| [`MEDIAN_LIFESTYLE_RESULT.md`](MEDIAN_LIFESTYLE_RESULT.md) | The 1,380 h/yr anchor. |
| [`RHO_SWEEP.md`](RHO_SWEEP.md) | The clearing-rate result and its calibration caveats. |
| [`DISPARITY_CEILING.md`](DISPARITY_CEILING.md) | The formal statement of `24/F` and its stress test. |

### Research notes

| Note | Bears on |
|---|---|
| [Labour and pollution intensity](../docs/GLOSSARY.md#src-labour-and-pollution-intensity) | §15 — the dollars-to-hours method, and why every figure is a floor. |
| [Consumer segmentation and archetypes](../docs/GLOSSARY.md#src-consumer-segmentation-archetypes) | §10 — where cohorts come from, and the rule that an archetype may name a cohort but never supply a number. |
| [Cross-country labour efficiency](../docs/GLOSSARY.md#src-cross-country-labour-efficiency) | §16 — what the locale dial must reproduce. |
| [Estimation-engine data sources](../docs/GLOSSARY.md#src-estimation-engine-data-sources) | §15 — the price-split ruling. |
| [Joint-production problem](../docs/GLOSSARY.md#src-joint-production-allocation-problem) | §6 — why divide-before-collapse matters. |
