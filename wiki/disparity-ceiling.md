...
**Formal statement + plain-language explainer:** [`06-simulation/DISPARITY_CEILING.md`](../sims/DISPARITY_CEILING.md). **Simulation** (7 self-tests green): [`06-simulation/disparity_ceiling_sim.py`](../sims/disparity_ceiling_sim.py).
...
- **Lower end `c ≥ F`** — staying alive is credited work ([[basic-needs-floor|§7.5]] self-care floor), so nobody sits at zero.
...
| **Dynasty / household** — pool N people into one mansion | A household is a co-op: its dwelling-debt splits per occupant by dwelling-time (children included, §6.2b). The bound is **per-person**; inheritance *dilutes* the load each generation. One member dominating the rest is coercion ([[service-to-influence|OP-1]]), not accounting. |
...
**Coupled to the pledge ruling:** the one channel that could have breached A3 — a *transferable pledged surplus* — was closed by making pledge surplus a non-consumable [[pledge-and-signal|contingent reserve]] (§6.4c). The disparity ceiling and permanent-pledges are one mechanism.
...
---
*Status: conditional result (consumption axis; conditional on OP-22 + narrow-band floor). Formally stated, simulated (7 tests), and stress-tested → PASSES 2026-08-14. Folded into Foundations v0.15 §7.5 / Objections v0.16 §C test 8.*
*Source: design session 2026-07-31 · formal statement `06-simulation/DISPARITY_CEILING.md`*
