...
> Every account may carry a baseline of debit with **no credit backing it** — an age-based floor that guarantees essentials unconditionally.

## What it is

The basic-needs mechanism. Three parts:

1. **Age-based baseline.** Every account carries a tolerance with nothing behind it. The formula should be derived by evaluating total production of essential goods and asking what an even distribution looks like.
2. **Essential provision is unconditional.** A mental health counselor is credited for providing service *regardless of the recipient's debit standing*. The provider is never made to means-test.
3. **Enforcement is graduated, not punitive.** Exceeding tolerance restricts **non-essentials only**. Essentials always flow.
...
- **OP-4 — the tolerance formula.** Parameterized for now, not derived.
- Who defines "essential"? *(unassigned)*

---
*Status: provisional — OP-4*
*Source: `00-strategy/Aequitas_Foundations_v0.19.md` §7.5*
