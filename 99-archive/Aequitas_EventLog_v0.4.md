...
> **Depends on:** `Aequitas_Foundations_v0.7.md` A1–A8
> **Supersedes:** `99-archive/Aequitas_EventLog_v0.3.md`. See §14. **v0.4 folds in the credit-realization session (Foundations v0.7):** credit realizes on *verification of the output* — for a good, the hand-off *is* the verification (§5.1b, IC-9); a pledge is a **1:1-backed pre-authorization of creditable work** and need not move debit (§5.1); **genesis is a distinct origin-terminus** for pre-Aequitas assets alongside a reservoir (§2, IC-3); a **deployment marker** starts holding-time (§2.2); the co-product split is **data-first** (§7.1a); wash-pledging is defused (§12.1). **No new primitive and no new field on `Event`.**
...
Neither needs a new primitive or a new field; both are ordinary `Event`s used in a particular way, called out because IC-3, IC-9, and the holding-time projection depend on recognising them.
...
This is a hard requirement on any implementation of the projection layer, and it closes a side entrance to OP-10 that was invisible before the allocation rule was written.
...
**Signals need no integrity constraint**, because nothing is conserved. Their failure mode is *flooding*, not imbalance, and flooding is a projection-side problem (OP-6).

### 5.1a Expiry

Unspent pledging power must revert, or the pool drifts down as pledges are made and forgotten. `expires_at` is mandatory. **Where reverted capacity goes — back to the pledger, or to a commons pool — is open and belongs to C5.**

### 5.1b Credit recording is ungated; realization gates on verification *(revised v0.4)*

**A pledge never gates the *recording* of credit.** An agent's work is logged as an event whether or not anyone pledged for it — gating *recording* on approval would contradict A7 and reproduce the origin-closure failure the A7 v0.2 amendment repealed (unpledged wheat must still have a grower for IC-3 to hold).
...
> **Schema consequence: none, and that is the correct outcome.** `AgentRole` already attaches at event level. **Projection consequence: severe.** A per-product labour-hour figure — which C3 requires, and which [EXIOBASE](https://www.exiobase.eu/) uniquely carries — cannot be derived from this schema without a declared convention. **That convention is OP-18, and it is now what blocks C3.**
...
**The A2 v0.3 amendment dissolves the question.** Training cost does not flow downstream at all, so there is no denominator to choose. **No field may make a past training event contribute debit to a later service event** — the guarantee is structural, like the absent `rate` field.
...
**IC-7 — Agent-time plausibility.** For any account and any window, the sum of its `AgentRole` intervals cannot exceed wall-clock time. **Nobody is credited with more than 24 hours of work per 24 hours.**
...
3. **The schema needed no change to accommodate the single most dangerous open problem in the project.** That is a reasonable signal C1 was right.
...
**IC-10 — Non-negative allocation.** No output's allocated share of any dimension is negative. A computed negative is a measurement error or a misdrawn process boundary, never a commodity containing less than nothing.
...
A7 requires records for flows nobody logged. These are **generated events**: `basis: modelled`, `resolution: cohort`, attributed to a cohort-proxy account.
...
The completion test for C1. Numbers are **illustrative placeholders**.
...
- **Externalities have nowhere to hide.** Fertilizer runoff, waste oil, and microplastics are ordinary output flows, structurally identical to the wheat (A4).
...
- **Nothing escapes.** Had E9 never been written, IC-4 would have reported 5 g unaccounted.

### 10.4 The sandwich already contained a joint process *(new in v0.3)*

**E3 — milling — is joint production, and v0.1 and v0.2 both walked past it.** 70 g of wheat plus 0.4 MJ yields 55 g of flour *and* 15 g of bran. Mass balances, so IC-1 passes — but **how much of the 0.4 MJ belongs to the flour?**
...
| **Vanishing material** | IC-4 fate closure; unaccounted mass is a reported query result | Closed structurally |
...
| **Granularity gaming** | `scope` forces a record to declare its own grain; conservative weighting makes vagueness expensive | Partly closed — C4 |
...
| **Debit dumping** | §5.2 — possession is the consent step; C6 removes throwaway accounts | **Closed for the crude form**; coerced transfer is a fraud problem, not a schema one |
...
|---|---|---|
| **Over-pledging** | IC-8 caps outstanding pledges at earned credit | Closed structurally |
| **Fabricated labour claims** | IC-7 caps at wall-clock; conservative weighting prices unattested claims at ~zero; pledges are the only route from asserted to backed | Closed |
| **Pledge parking** | `expires_at` is mandatory | Closed structurally |
| **Signal flooding** | Nothing in the schema; signals are unbacked by design | **Open — OP-6** |
| **Wash-pledging / wash-trade rings** | **Real deployable work dominates it** *(v0.4)*: real work *sheds* the debit (a buyer takes it) while a wash-trade *retains* it (the make-and-keep self-work identity nets ~zero, Foundations §3.2), plus real overhead is pure loss. Colluders end with ~zero net contribution, a wrecked ratio, and debt they can only shed by dumping (OP-25). The residual — colluders manufacturing *pledging-power/influence* from gross fake hours — is bounded by IC-7 and routes to OP-1, not here. | ✅ **Defused** (residual → OP-1) |
...
| **Boundary gerrymandering** — split one process into two to change the base | **IC-12** — stage-by-stage allocation must equal whole-process allocation. Detectable arithmetic, not an arguable judgement. | ✅ Closed |
| **Negative shares** | **IC-10** — asserted, not proven for the recursive case | ⚠️ Mitigated |
...
| **Overhead stuffing** — reclassify traceable inputs as overhead to escape into the soft inheritance rule | **Nothing yet.** §1.1 makes the choice visible; nothing makes it expensive. | 🔴 **Open — OP-23** |
| **Constant capture** — publish energetics favourable to your sector | Rival-sector audit + two unaffiliated replications before re-weighting history (Foundations §3.3a). Unproven. | 🔴 **Open — OP-24** |
...
4. **Reservoir registry** — fate closure (IC-4) is only as good as this registry.
5. ~~**Amortization denominator**~~ — ✅ resolved in v0.2.
6. **Confidence propagation rule** — how basis, confidence, and resolution combine along a parcel DAG. **Prerequisite for C3 and C4.** *Now also needs to cover allocated shares, whose confidence derives from process-energetics data quality rather than from the parent measurement alone (§4.1).*
...
9. **Pledge reversion target** (§5.1a) — C5.
10. **Process-energetics model registry** *(new)* — the published per-process data that §7.1a's splits are computed from, plus the replication and rival-audit rules of Foundations §3.3a. **This is a new external dependency created by resolving item 8, and it is where OP-24 lives.**
11. **Labour allocation convention across co-products** *(new)* — §6.1. **This is OP-18, and it is what now blocks C3.**
...
3. **§5.1 reframed — the Pledge/Signal distinction is the 1:1 `hours` backing (IC-8), not "transfers debit."** A pledge is a pre-authorization of creditable work and may move no debit (the public-verge case).
4. **IC-9 clarified** — discharge moves property-debit only when the work yields a held object; a pure service moves none.
5. **§2.2 added — genesis entries and deployment markers**, both ordinary events. **IC-3 now accepts a genesis terminus** (pre-Aequitas asset) alongside a reservoir extraction — a legitimate root that is *not* a reservoir.
...
7. **§12.1 — wash-pledging/wash-trade upgraded from *Mitigated* to *Defused*** (real work dominates it; residual routes to OP-1).

*Unchanged: every primitive, every field of `Event`, `Flow`, `Parcel`, `Reservoir`, `Account`, the three detail axes, IC-1 through IC-8, IC-10 through IC-12, supersession monotonicity, and the sandwich trace.*

## 14-prev. Changes in v0.3
...
2. **§3.1 added — debit is a vector and splits happen per dimension before collapsing.** Hard requirement on the projection layer; closes a side entrance to OP-10.
3. **§7.2 added — IC-10, IC-11, IC-12**, the first **projection-side** integrity constraints in the spec. Non-negativity (asserted, not proven), exhaustiveness, and boundary additivity.
4. **§6.1 added — labour does not allocate across co-products**, and this is now what blocks C3. Schema consequence: none. Projection consequence: severe.
...
10. **§12.2 added** — attack table for the allocation rule. Two exploits closed by construction, two open (OP-23, OP-24).
