...
> **Depends on:** `Aequitas_Foundations_v0.13.md` A1–A8
> **Supersedes:** `99-archive/Aequitas_EventLog_v0.5.md`. **Conforms the pledge records to Foundations v0.13 (pledge-mechanics correction) + presentation cleanup.** A pledge is a **revocable grant of debit-room** (virtual credit conferred on its target, backed 1:1 by the pledger's own credit), **not** a pre-commitment to take possession. Three record-level consequences: a pledge now carries a **`retracted_by`** field (it may be withdrawn at any time before it is spent); **IC-9** no longer moves a pledged object's debit "to the pledger" — the object's debit follows possession to *whoever accepts it* (§5.2), which need not be the pledger; and the pledger's underlying credit is never consumed — only *pledging-power* is committed, released on retraction/expiry and spent when the summoned work occurs. Also: inline version-tags stripped, a Contents list added, and the in-document change log (§14) moved to `Aequitas_EventLog_CHANGELOG.md`.
...
<!-- tag: evt-s2-2 -->
### 2.2 Two special events — genesis and deployment
...
Pledge {
  ...
  hours       : Duration,           // pledging-power committed, backed 1:1 by the pledger's earned credit
  expires_at  : Instant,            // §5.1a — unspent pledges must revert
  discharged_by : EventId?,         // set when the summoned work occurs (pledging-power spent)
  retracted_by  : EventId?,         // set if the pledger withdraws before discharge (§5.1a) — pledges are revocable at any time
  timestamp, signature
}
...
**The distinguishing property is structural, not a flag: a `Pledge` carries `hours` backed 1:1 by earned credit (IC-8 (pledge backing)); a `Signal` does not.** A pledge is a **revocable grant of debit-room** — a 1:1-backed pre-authorization of creditable work that confers *virtual credit* on its `target` at projection time, giving that target more room to carry a cost (Foundations §6.4, §6.2b). Three properties matter at the record level:

- **It consumes no credit.** The pledger's earned credit never moves and is never earmarked; what a pledge commits is *pledging-power* (a spendable allowance equal 1:1 to earned credit, IC-8). Committing it only means it cannot be pledged elsewhere until released.
- **It is revocable.** A pledger may withdraw an undischarged pledge at any time (`retracted_by`), which returns the committed pledging-power. A cooperative therefore cannot treat a pledge as a guaranteed sale.
...
<!-- tag: evt-ic-8 -->
**IC-8 — Pledge backing.** For any account, the sum of its outstanding (undischarged, unexpired, unretracted) pledged hours may not exceed its earned credit hours. This holds the pledging-power an account grants at or below the credit backing it — the record-level form of "no fractional-reserve pledging" (Foundations §6.4).

<!-- tag: evt-ic-9 -->
**IC-9 (pledge discharge) — Pledge discharge.** When the summoned work occurs, `discharged_by` is set to the event and the committed pledging-power is spent. **If the work yields a held object**, that object's property-debit moves under the ordinary custody rule (IC-5) to **whoever accepts possession** — the pledge does not compel the pledger to receive it, and taking it is a separate custody act on the accepter's own debit-room (§5.2). **If it is a pure service or public good** (mowing a public verge), no property-debit moves — the pledge simply spent pledging-power to summon creditable work. Either way the discharge is capped by IC-8, and an undischarged pledge may instead be **retracted** (`retracted_by`), returning the pledging-power.

> **⚠️ A pledge is not a pre-commitment to take possession.** Custody is decided by physical possession (§5.2); there is no ledger-level obligation created by a pledge. A pledge confers *debit-room* on its target and is revocable at any time (Foundations §6.4/§6.2b); it never binds the pledger to accept an object. Any earlier reading of a pledge as "the affirmative case of a custody-refusal right" is void — there is no such right.
...
Unspent pledging power must revert, or the pool drifts down as pledges are made and forgotten. `expires_at` is mandatory. A pledge may also be **retracted** before discharge (`retracted_by`, §5.1) — the same return of committed pledging-power, triggered by the pledger rather than the clock; pledges are revocable at any time (Foundations §6.4). **Where reverted or retracted capacity goes — back to the pledger, or to a commons pool — is open and belongs to C5 (debit taxonomy).**
