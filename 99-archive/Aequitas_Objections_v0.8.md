...
> **Version:** 0.8 · **Date:** 2026-08-06
> **Supersedes:** `99-archive/Aequitas_Objections_v0.7.md` — **the credit-realization session (C11 interview).** Hand-off-gates-credit stress-tested to PASSES WITH CHANGES; all three credit-realization exploits defused (new **B10**). **P4 weakened again** (mislabeling defused by the public pledge ledger; the wage-extraction employer is structurally hollowed out); **OP-22 made more load-bearing** (the market-public/persons-private principle depends on it); a new **OP-1 sub-question** (influence from gross fake hours) inherits the one surviving residual.
> **Tracks:** `Aequitas_Foundations_v0.7.md` · `Aequitas_EventLog_v0.4.md` · `OP-18_labour_and_team_credit.md` · `OP-23_capital_and_pollution.md` · `OP-17_coproduct_allocation.md` · `06-simulation/RESULTS.md`
...
| | Item | Status |
|---|---|---|
| 🔴 | **OP-10 / P8 — Weighting-model governance** | Open. One side entrance closed (§3.2a); cost constants have a mechanism (§3.3a); the general problem stands. **Now the top blocking problem.** |
| 🔴 | **OP-24 — Understatement drift** | Errors favouring subscribers have no funder. Attacks A4. **Lever enlarged in v0.6** by the stock/baseline constants. |
...
| 🟠 | **P4 — Coordinator class** | **Weakened again in v0.8.** Credit-accumulation form dead; **wage-extraction employer now structurally hollowed out** (no wages A3, no surplus A5, no rank-based dumping); mislabeling defused (public pledge ledger). Only the *coordination* residual (who holds empowering work; hours-inequality in pledging) survives. |
| 🟠 | **OP-1 — Service → influence** | **Now carries the credit-realization residual:** gross fake hours from collusive hand-offs could pump pledging-power — bounded by IC-7, paid in ratio, possibly self-starving. See B10. |
| 🟠 | **OP-9 / P5 — Preference revelation** | **Substantially answered.** Needs writing up. |
| 🔽 | **OP-22 — Minimum audit disclosure** | Narrowed to a C7 implementation question — **but more load-bearing after v0.8:** the market-public/persons-private transparency principle depends on it. |
...
| 🔽 | **OP-25 — Illicit end-of-life dumping** | **New.** §3.6 prices lawful disposal; abandonment attribution is a Level-2 problem. |
| ✅ | **OP-18 — Responsibility is not divisible** | **CLOSED in v0.7** as the C3 blocker — team-credit dissolves under A2; labour rides the material split; cost ≠ scarcity. See **B9**. |
| ✅ | **OP-23 — Shared-overhead attribution** | **CLOSED in v0.6** — capital and overhead never allocate to co-products. See **B8**. |
...
**Fourteen items closed; six live.** The pattern held for a fourth session and then broke on the fifth in an instructive way: OP-17, OP-11, OP-21, OP-23 all closed by *removing* a division. **OP-18 is the first to close by *declaring* a convention** — because labour genuinely leaves no trace, the physical-trace test *mandates* a convention here. **v0.8's contribution is different again:** the credit-realization model (B10) closed a cluster of *incentive* exploits not by removing or declaring anything, but by discovering that a mechanism the author already had — **debit-follows-possession** — inverts the attacker's leverage. Each of the three exploits died against an existing rule, and the one surviving residual is an *influence* question (OP-1), not a credit-accounting one.
...
**What remains genuinely indivisible is now vanishingly small.** After v0.7, of the two OP-18 scales: **labour across co-products** is closed — it gets a *declared convention* (rides the material split, §3.4a/§1.1), which is exactly what the physical-trace test prescribes for a no-trace quantity. **Responsibility across a team** dissolved for *credit* (A2: own hours) and survives only as the apportionment of a jointly-*caused debit* (team pollution/harm) — minor, non-blocking, sibling to OP-25. **The untraceable residue is no longer a blocker anywhere.**
...
**Status:** ✅ **CLOSED 2026-08-05 as the C3 blocker — moved to Part B (B9).** The statement below is kept as the record of the problem; the resolution is in B9. Resolution note: `00-strategy/OP-18_labour_and_team_credit.md`.
...
> **⚠️ Lever enlarged in v0.6.** Foundations §3.3 now makes pollution-debt **stock-dependent**, floating with the ambient stock above a natural-remediation baseline. Two new constants — the **equilibrium baseline** and the **ambient-stock measurement** — move every pollution record in history at once. They are the largest single levers in the weighting model, and the consumer-side understatement bias below applies to them with full force: everyone benefits from the baseline being set high (fewer things count as pollution) and the stock being read low. Foundations §3.3a places them explicitly under rival-sector audit. **This does not solve OP-24; it raises the stakes on it.**
...
## A4 — OP-23. Shared-overhead attribution ✅ **CLOSED in v0.6 → moved to B8**
...
## A11 — OP-25. Illicit end-of-life dumping *(new in v0.6)*

**Attacks:** A4, at the disposal end.
...
> **A durable asset holds its own creation-cost as property-debit *on the asset*. It is drawn down by community pledges first, and any residual is holding-time-split among its holders. It never flows to the co-products at all.**

The barn stays on the farm operator; the fab stays on its cooperative; hide and beef and each wafer carry only their own consumables. **There is nothing to attribute, so there is no attribution convention to get wrong and nothing for overhead-stuffing to exploit** — the exploit dies with the allocation it was gaming. This is the same move that closed OP-11 and OP-21: *a division problem dissolved by removing the division.*

**Why it does not leak (A4).** The overhead debit is not lost; it is **located** on the asset and its holders rather than smeared across units. Honest trade-off, stated in §6.2b: a per-unit debit-cost is therefore not a full-lifecycle figure.
...
**Spawned:** **OP-25** (illicit end-of-life dumping — a Level-2 attribution problem) and an enlarged lever for **OP-24** (the new stock/baseline constants).
...
**Method:** opened → separated into two sub-problems → one dissolved against A2, the other resolved by the physical-trace test with the split basis chosen by the author, then axiom-scored and stress-tested.

**The objection.** Ellerman: responsibility for joint work is joint and non-decomposable — there is no fact saying the welder caused 40% of the bridge, nor how many of the farmer's hours are "in" the hide. OP-17 split materials and energy by physical trace; **labour has no such trace.** C3 needs per-product labour hours.

**The answer — it is two problems, and they close two different ways.**

> **(β) Team credit dissolves under A2.** Credit is *time worked* (§6). Each member is credited **their own hours** — the "40% of the bridge" number is never required. Credit was never a share of output; the objection conflated *credit-for-hours* with *share-of-responsibility*. The axiom already answered it. *(Residue: apportioning a jointly-caused debit — team pollution/harm — across members. Minor, non-blocking, sibling to OP-25.)*
>
...
**Shipped:** Foundations v0.7 §6.4 (pledge broadened), §6.4a (hand-off model), §3.2 (debit-taxonomy refined), §6.2b (deployment/transit), §6.2a (pre-Aequitas genesis), §3.7 (land remediation), §5.3 (transparency principle), §7.1 (employer hollowed out); EventLog v0.4 §7.3, §5.1b, §2.2, IC-9, §12.1.
...
**Side findings banked:** the wage-extraction **employer is structurally hollowed out** (→ A7/P4); **demand risk is symmetric by hours**, not dumpable by rank (→ A7); the **market-public/persons-private** transparency principle (→ A9/OP-22).

**The one surviving residual.** Realized credit → pledging-power (influence) is measured in *gross hours*, so a consumption-indifferent zealot could collude to fake gross hours and pump influence — bounded by IC-7 (24 h/day), paid in a wrecked ratio, and possibly self-starving (pledging costs debit-room, §6.2b). **This is an OP-1 influence question, not a credit-accounting flaw.** Tests owed: §C.
...
> **✅ Confirmed in v0.6 — OP-23 closed by exactly this.** Tooling and plant *are* front-loaded and pledged rather than amortized into output. Overhead does not shrink — it stops being allocated to outputs at all (§6.2b). The bet recorded here paid out. See **B8**.
...
7. **Influence back-door** (B10 / OP-1). Simulate collusive hand-offs manufacturing gross fake hours → pledging-power, against the IC-7 cap and the debit-room cost of pledging. Find whether the ratio/debit-room penalties make it self-defeating, or whether a consumption-indifferent actor can still buy influence.
...
2. **P4 (A7) weakened further.** The wage-extraction employer is structurally hollowed out (no wages A3, no surplus A5, no rank-based dumping §6.2b); mislabeling defused by the public pledge ledger; demand risk symmetric-by-hours and floored. Only the coordination residual survives.
3. **OP-1 gains the influence back-door** — the one surviving residual (gross fake hours → pledging-power) routes here, not to credit accounting.
4. **OP-22 (A9) made more load-bearing** — the market-public/persons-private transparency principle depends on it.
5. **Status board: fourteen closed, six live.** §C gains two tests (demand-risk floor adequacy; influence back-door).

## E-prev. Changes in v0.7

1. **OP-18 closed as the C3 blocker and moved to Part B as B9.** Team-credit dissolves under A2 (own hours); labour-across-co-products rides the material split by declared convention; co-product cost is embodied input, not scarcity (the tenderloin case; Method 2 yield-weighting rejected as price-rationing).
...
3. **Status board: thirteen closed, six live.** OP-10 is now the top blocking problem.
...
## E-prev. Changes in v0.6

1. **OP-23 closed and moved to Part B as B8.** Capital and overhead accrue to the asset and never allocate to co-products (§6.2b); the interim inherited-proportions convention was deleted, and the overhead-stuffing exploit died with it.
2. **§0 narrowed again** — the indivisible set drops from three items to two (both OP-18). Overhead left by relocation, not by splitting.
3. **OP-25 added (A11)** — illicit end-of-life dumping, a Level-2 attribution problem, explicitly minor.
4. **OP-24's lever enlarged (A3)** — the new stock-dependence and equilibrium-baseline constants (§3.3) are the largest single levers in the weighting model and sharpen, not solve, the understatement problem.
5. **A4 (OP-23) converted to a tombstone** pointing at B8; B3 and the status board updated to record the closure.

## E-prev. Changes in v0.5
