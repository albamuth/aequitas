...
> **Version:** 0.13 · **Date:** 2026-08-10
> **Supersedes:** `99-archive/Aequitas_Objections_v0.12.md` — **the scenario-suite fold.** §C **test 8's consumption-axis real-distribution comparison is now DONE** (`06-simulation/q4_locked_ledgers.py`): entering the population **material-only** (A1 corollary — financial instruments carry no debit) compresses the observed inequality tail **~1,000×** (wealth ~10⁶× median → material consumption ~670×), and only **~0.1–2%** sit past a permanent efficiency-ratio lockout — the ultra-consumers, not the merely rich; ~two-thirds gain room by joining. Plus the standing cross-cutting result from the suite: **labour is abundant; materials/energy bind** (`q1_autarky.py`, `q5_reallocation.py`). No new objection, no mechanism change — supporting evidence for the §7.5 ceiling and §3.5. Prior fold (v0.12) below.
> **Prior (v0.12):** the disparity-ceiling simulation fold. §C **test 8 moved "owed" → "simulated"**: `06-simulation/disparity_ceiling_sim.py` shows the `24/F` ceiling exact and **ρ-independent** (and weighting-independent — not dependent on OP-10), a **ρ that clears the market and tightens under shocks**, and **fraud-invariance** (IC-7 caps every account at 24 h/day). Still **conditional on OP-22**. Prior fold (v0.11) below.
> **Prior (v0.11):** the electricity-attribution fold. New stress-test verdict **B12 — the §3.2b real-time-dispatch principle (electricity generation → consumer), PASSES WITH CHANGES.** The raw "all generation pollution to the consumer" was caught weakening the decarbonisation incentive on a pooled grid; the adopted fix attributes by the consumer's **contracted supply mix** (provenance, §5.1b). Grid emission factor → **OP-24 (understatement drift)** / §3.3a; universality edge (real-time-vs-batch spectrum) flagged open.
> **Prior (v0.10):** the OP-9 write-up fold. **OP-9 (calculation reply) / P5 (preference revelation) downgraded 🟠→🟢**: the four-move reply (cost≠value · pledges reveal demand · tractability cited · scarcity-as-debit without an objective function) now lives in the standalone `00-strategy/OP-9_calculation_reply.md`, with the plain-language version folded into Overview §9. **OA8 rewritten** — the scarcity-as-debit and Hayek residues both **terminate in OP-10 (weighting governance)**, registered there, not claimed closed.
> **Prior (v0.9):** the work-definition session. The self-care / definition-of-work cluster stress-tested to **PASSES** (**B11**): *as an instance of OP-10/OP-22, not a new hole*. **OP-4 (debit tolerance) promoted**; **OP-1 (service → influence)** gains the **universal-basic-voice** result; **OP-14 (cohort shopping)** gains floor/routing shopping; **OP-8 (feedback firewall)** sharpened; **OP-10 (weighting governance)** gains its highest-leverage instance (the self-care constant).
> **Tracks:** `Aequitas_Foundations_v0.12.md` · `Aequitas_EventLog_v0.5.md` · `OP-18_labour_and_team_credit.md` · `OP-23_capital_and_pollution.md` · `OP-17_coproduct_allocation.md` · `06-simulation/RESULTS.md` · `06-simulation/DISPARITY_CEILING.md` · `06-simulation/scenario_suite_METHOD.md`
...
| 🟠 | **OP-16 (onerousness gap) — Onerousness gap** | **Half answered.** Skill half fixed by A2 (time as measure) v0.3. Tedium half open. |
...
|---|---|---|---|
| **1 — Valuation** | Flat-hour crediting cannot recruit skill | Warren; **time banking, 45 years** | 🟠 Skill half fixed; **OP-16 (onerousness gap)** open |
...
<!-- tag: obj-oa3 -->
## OA3 — OP-24. Understatement drift *(new in v0.5)*
...
**Sources:** [Mises (1920)](https://mises.org/library/economic-calculation-socialist-commonwealth) · [Hayek (1945)](https://www.econlib.org/library/Essays/hykKnw.html) · `02-research/neurath-calculation-in-kind.md` · [Dapprich](https://brightagebeyond.com/wp-content/uploads/2022/05/dapprich-philipp-2021-optimal-planning-with-consumer-feedback-a-simulation-of-a-socialist-economy.pdf) · `02-research/kantorovich-shadow-prices.md` · `02-research/cockshott-cottrell-labour-time.md`
...
<!-- tag: obj-oa10 -->
## OA10 — The auditor-independence problem *(new in v0.5)*
...
<!-- tag: obj-oa11 -->
## OA11 — OP-25. Illicit end-of-life dumping *(new in v0.6)*
...
**The one surviving residual.** Realized credit → pledging-power (influence) is measured in *gross hours*, so a consumption-indifferent zealot could collude to fake gross hours and pump influence — bounded by IC-7 (24-hour cap) (24 h/day), paid in a wrecked ratio, and possibly self-starving (pledging costs debit-room, §6.2b). **This is an OP-1 influence question, not a credit-accounting flaw.** Tests owed: §C.
...
<!-- tag: obj-e-changes-in-v0-12 -->
## E-prior. Changes in v0.12
...
<!-- tag: obj-e-changes-in-v0-9 -->
## E-prev. Changes in v0.9
...
<!-- tag: obj-e-prev-changes-in-v0 -->
## E-prev. Changes in v0.8
...
<!-- tag: obj-e-prev-changes-in-v0-2 -->
...
<!-- tag: obj-e-prev-changes-in-v0-3 -->
...
<!-- tag: obj-e-prev-changes-in-v0-4 -->
