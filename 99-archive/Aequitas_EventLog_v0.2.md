...
> **Depends on:** `Aequitas_Foundations_v0.3.md` A1–A8
> **Supersedes:** `99-archive/Aequitas_EventLog_v0.1.md`. See §13.
...
## 5.1 Pledges and signals — new in v0.2
...
**The distinguishing property is structural, not a flag: a `Pledge` transfers debit and a `Signal` does not.** A signal has no `hours` field and never touches a ledger — there is no field in which it could, which is the same style of guarantee as §9's absent `rate`.

**Two new integrity constraints follow, and both are pure arithmetic:**

**IC-8 — Pledge backing.** For any account, the sum of its outstanding (undischarged, unexpired) pledged hours may not exceed its earned credit hours. This is what forbids fractional-reserve pre-ordering.

**IC-9 — Pledge discharge is a transfer.** When pledged work occurs, the resulting property debit moves to the pledger under the ordinary custody rules (IC-5). A pledge is the **affirmative case of custody acceptance** — the same rule that lets a transfer be refused — so it introduces no new transfer semantics.

**Signals need no integrity constraint**, because nothing is conserved. Their failure mode is *flooding*, not imbalance, and flooding is a projection-side problem (OP-6), not a schema one.
...
**A pledge never gates credit.** An agent is credited for work performed whether or not anyone pledged for it; what the pledge supplies is a committed counterparty for the resulting debit. Gating credit on approval would contradict A7 and reproduce the origin-closure failure that the A7 v0.2 amendment repealed — unpledged wheat would have no grower, and IC-3 would fail on a parcel whose production was real.
...
- **Skilled labor** → training is **credited work in its own right**, recorded as ordinary events at the time it occurs. Its debit is discharged then, by whoever pledged for it (§5.1). `capacity_ref` still points at the training provenance, but it is now **attribution and audit metadata only** — it feeds no downstream calculation.

### ✅ Resolved in v0.2: the amortization denominator

v0.1 flagged that `capacity_ref` said *what the training cost* but not **over how many future service-hours that cost spreads**, and that A2 was not implementable until it was settled. Every candidate denominator — expected career hours, actual career hours (knowable only posthumously), a statutory window — had a defect.

**The A2 v0.3 amendment dissolves the question.** Training cost does not flow downstream at all, so there is no denominator to choose. A skilled service costs its recipient the provider's time plus real materials, and nothing else.

**Schema consequence: nothing is added, and one thing must be forbidden.** No field may make a past training event contribute debit to a later service event. `capacity_ref` is a link for provenance, not a term in a cost function — and like the absent `rate` field (§9), the guarantee is structural rather than a rule anyone must remember to follow.

*This is the same reasoning that removed the analogous divisions for media production and research: a large up-front cost with diffuse benefit is front-loaded and pledged, never amortized. See Foundations §6.2a.*
...
> **⚠ Escalated in v0.2 — conservative weighting is now load-bearing beyond granularity.** Foundations §6.6 relies on it to close OP-20: an unwitnessed, near-zero-material claim of creative hours is recorded faithfully and, weighed at the pessimistic end of its interval, is worth close to nothing until something corroborates it. **The same rule that makes vague industrial recording expensive is what makes fabricated labour claims worthless.** It has gone from an incentive nicety to a defence the theory depends on, and it should be specified early in C4 rather than late.

### 12.1 Attacks on the new records

| Attack | Structural answer | Status |
|---|---|---|
| **Over-pledging** — commit more debit than you can absorb | IC-8 caps outstanding pledges at earned credit | Closed structurally |
| **Fabricated labour claims** — assert unobservable hours | IC-7 caps at wall-clock; conservative weighting prices unattested claims at ~zero; pledges are the only route from asserted to backed | Closed — see Foundations §6.6 |
| **Pledge parking** — lock capacity in pledges that never discharge, to look committed | `expires_at` is mandatory (§5.1a) | Closed structurally |
| **Signal flooding** — mass-generate content and mass-signal it | Nothing in the schema; signals are unbacked by design | **Open — OP-6, projection-side** |
| **Wash-pledging rings** — pledge reciprocally to manufacture activity | A1 still forces real matter to move, and the resulting goods carry real property debit that someone must hold | Mitigated; overlaps OP-2 |
...
5. ~~**Amortization denominator** for `capacity_ref`~~ — **✅ resolved in v0.2.** The A2 amendment removes the downstream flow entirely; there is no denominator to choose (§6).
...
7. **Conservative weighting of low-confidence flows** (§12) — **escalated**: now the defence behind OP-20, not merely a granularity incentive. Belongs to C4, early.
8. **Co-product allocation convention** *(new)* — when one event has several outputs, the schema records the split faithfully but **does not say how to compute it, and no physical rule does.** `basis: allocated` marks that a split occurred; which split is correct is **OP-17**, and it blocks C3. This is the single largest gap between the schema and a runnable cost figure.
9. **Pledge reversion target** (§5.1a) — where expired pledging capacity goes. C5.

Items 1–4 are ordinary standards work. **Items 6, 7, 8 and 9 are theory work.** Item 8 is the blocking one.

---

## 14. Changes in v0.2

1. **§6 — the amortization denominator is resolved and removed**, following the A2 amendment in Foundations v0.3. Training is credited work discharged at the time it occurs; `capacity_ref` becomes provenance metadata that feeds no cost function. **A2 is now fully implementable**, which v0.1 explicitly said it was not.
2. **§5.1 added — `Pledge` and `Signal`**, with **IC-8** (pledge backing) and **IC-9** (discharge is an ordinary custody transfer). The distinguishing property is structural: a pledge moves debit, a signal has no field with which to.
3. **§5.1b added** — pledges never gate credit, or IC-3 origin closure would fail on real production, reproducing exactly the defect that repealed A7 v0.1.
4. **§12 — conservative weighting escalated** from a granularity incentive to a load-bearing defence (OP-20).
5. **§12.1 added** — attack table for the new record types.
6. **§13 item 8 added** — co-product allocation named as the blocking gap (OP-17). v0.1 recorded that allocation happens via `basis: allocated` without noticing that **which** allocation is correct has no physical answer.

*Unchanged: every primitive, every field of `Event`, `Flow`, `Parcel`, `Reservoir`, `Account`, the three detail axes, IC-1 through IC-7, supersession monotonicity, and the sandwich trace. The v0.1 structure survives the Foundations v0.3 amendments intact — which is a reasonable signal that the schema was right.*
