...
> **Supersedes:** `99-archive/Aequitas_Overview_v0.5.md`. v0.6 folds in the **OP-9 socialist-calculation reply** (the Mises/Hayek attack every economist brings first). §9's thin one-paragraph treatment of "you can't run an economy without prices" is replaced by a proper plain-language answer: Mises aims at *value*, which Aequitas never computes; demand is revealed by pledges, not a committee; Hayek's knowledge point is an objection to central *planning*, which Aequitas isn't; and tractability is settled and cited. The rigorous version is the new `00-strategy/OP-9_calculation_reply.md`. Structure otherwise unchanged.
...
**The "you can't run an economy without prices" objection** *(expanded in v0.6)*. This is the oldest and most cited attack — every economist brings it first, so it deserves a real answer rather than a wave. It comes in two flavours, and they need different replies.
