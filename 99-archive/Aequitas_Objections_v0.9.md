...
> **Version:** 0.9 · **Date:** 2026-08-07
> **Supersedes:** `99-archive/Aequitas_Objections_v0.8.md` — **the work-definition session.** The self-care / definition-of-work cluster stress-tested to **PASSES** (new **B11**): *as an instance of OP-10/OP-22, not a new hole* — network weighting-pluralism is legitimate (A6/§3), and the anti-arbitrage guard is **counterparty re-computation** (comparison, never conversion), which depends on OP-22 (audit disclosure). **OP-4 (debit tolerance) promoted** (now the denominator of the disparity ceiling; no global debit:credit ratio exists — §3.5/A8 forbid it); **OP-1 (service → influence)** gains the **universal-basic-voice** result (self-care credit bounds influence disparity to `24h ÷ floor`); **OP-14 (cohort shopping)** gains self-care-floor / routing shopping; **OP-8 (feedback firewall)** sharpened (feedback ≠ verification); **OP-10 (weighting governance)** gains its highest-leverage instance — the self-care constant (universal + influence-bearing).
> **Tracks:** `Aequitas_Foundations_v0.8.md` · `Aequitas_EventLog_v0.5.md` · `OP-18_labour_and_team_credit.md` · `OP-23_capital_and_pollution.md` · `OP-17_coproduct_allocation.md` · `06-simulation/RESULTS.md`
...
| 🟠 | **OP-9 (calculation reply) / P5 (preference revelation) — Preference revelation** | **Substantially answered.** Needs writing up. |
...
**Attacks:** A5 (price ≡ cost). The Mises/Hayek line of attack.
...
2. **Scarcity-as-debit handles rationing.** On [Kantorovich's](https://www.nobelprize.org/prizes/economic-sciences/1975/kantorovich/lecture/) objectively determined valuations, a shadow price is the **cost of a binding constraint**, not a margin extracted by a seller. **Recorded as debit rather than skimmed as margin, this is compatible with A5 and arguably required by A4 (no externalities).**

**P5 (preference revelation) — the Cockshott variant.** Their demand lever is the *gap* between market-clearing price and labour value. **A5 collapses that gap to zero by construction**, so Aequitas inherits their problem with one fewer instrument. **Pledges are the replacement.**

**Remaining catch:** a dual price requires a primal optimisation, which requires an objective function — **straight into OP-10 (weighting governance).** Federating it per-constraint may help, since most scarcity constraints are local (this lake, this ore body). **Unverified.**

> **⚠️ Sharpened in v0.5 — the OP-17 (joint production) session confirmed the danger.** A demand-contingent allocation rule was proposed and rejected: splitting a steer's debit by which cuts are sought-after makes two identical steers in two towns carry different splits, which **fails universality and is price allocation in costume.** The general lesson: **whenever demand is invited into the cost side, check whether A5 has been reintroduced under a new name.** OP-9's answer must keep scarcity strictly as a *material* cost.
...
8. **Generous-network race + the disparity ceiling** (B11 / OP-14 (cohort shopping) / OP-24 (understatement drift) / OP-4 (debit tolerance)). Sim N trust networks setting different self-care floors, with agents free to cohort-shop; does the floor race to the IC-7 ceiling, or does counterparty re-weighting (§6.4b) arrest it? Then the disparity-ceiling proof itself: the accrual bound is `24 h ÷ floor`, within-model — simulate the resulting consumption-ceiling distribution under a stated fraud rate × magnitude, and compare it to a real wealth (Pareto) tail. **This is the strongest defensive result the project can hold**, and it now rests on Foundations v0.8 (floor = credited time; OP-4 shape).
