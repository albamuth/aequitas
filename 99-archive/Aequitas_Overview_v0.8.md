...
> **Supersedes:** `99-archive/Aequitas_Overview_v0.7.md`. **Electricity-attribution fold (§4).** One reader-facing addition: **the pollution from generating your electricity is *yours*, not the power plant's** — because electricity is made the instant you draw it, so your usage commands the generation, exactly as your car's tailpipe is yours. You bear the emissions of *the supply you contracted for*, so choosing a cleaner provider lowers your debit and pushes generators to clean up. *(Prior: v0.7 embedded the debit-taxonomy schematic in §1 and corrected the disparity-ceiling claim to a conditional result. v0.6 folded in the OP-9 socialist-calculation reply.)*
> **Rigorous version:** `00-strategy/Aequitas_Foundations_v0.11.md`. This document is the plain-language companion; where they differ, the Foundations govern.
...
*For the rigorous statement of everything above, with the arguments and the open problems in full: `00-strategy/Aequitas_Foundations_v0.11.md`.*
