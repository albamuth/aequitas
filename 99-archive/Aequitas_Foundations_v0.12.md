...
> **Supersedes:** `99-archive/Aequitas_Foundations_v0.11.md`. **Scenario-suite fold (A1 corollary, §3.5, §7.5).** No mechanism change — one explicit axiom corollary and two societal-scale empirical results from `06-simulation/` (the five-sim scenario suite): **(1)** *(A1)* **financial instruments — stocks, bonds, crypto, options — are abstract/fiat and carry no debit;** only the underlying *material* assets do, and those sit on whoever physically holds or operates them (§3.2/§6.2b). Material is counted once, on the actual holder — no double-counting, and no loophole (owning a factory through shares still charges the factory's *operators* by holding time). This was always implied by A1's "no abstract, issued, or fiat quantity"; it is now stated. **(2)** *(§7.5)* the disparity ceiling gains real-distribution backing: entering the population **material-only** compresses the observed inequality tail **~1,000×** (money wealth reaches ~10⁶× the median; material consumption only ~670×, because physical consumption is time- and biology-bounded), and only **~0.1–2%** of people sit past a permanent efficiency-ratio lockout (`q4_locked_ledgers.py`). **(3)** *(§3.5)* **labour is not the scarce factor** — self-care is credited work (~3.4× all productive labour), so an autarkic US or the world economy is bound by *materials and energy*, not hours (`q1_autarky.py`, `q5_reallocation.py`). Prior fold (v0.11) below.
> **Prior:** v0.11 superseded v0.10 — **disparity-ceiling simulation fold (§7.5).** No mechanism change — the ceiling gained **simulation backing** (`06-simulation/disparity_ceiling_sim.py`, companion `DISPARITY_CEILING.md`): the `24/F` bound is exact and **ρ-independent** (and weighting-independent, so it does **not** depend on OP-10); a **ρ clears the market and moves like a prime rate** under shocks; and the ceiling is **fraud-invariant** because IC-7 caps every account at 24 h/day. Still **conditional on OP-22** (the sim assumes the anti-arbitrage guard). Prior fold (v0.10) below.
...
> **Companion:** `00-strategy/Aequitas_Objections_v0.13.md` — the objections register. Read alongside §10.
...
> **The real-distribution comparison, now run** *(new in v0.12)*. `06-simulation/q4_locked_ledgers.py` applies the ceiling to real US/world distributions under the **material-only** rule (A1 corollary above), asking what fraction would sit past a *permanent* efficiency-ratio lockout — discretionary consumption pinned to the floor for life because their sustained footprint exceeds `ρ · 24 h/day`, the most any human can ever earn. Two results sharpen §7.5:
...
Ranked by how load-bearing they are. Full detail in `00-strategy/Aequitas_Objections_v0.13.md`.
