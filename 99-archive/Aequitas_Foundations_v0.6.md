...
> **Supersedes:** `99-archive/Aequitas_Foundations_v0.5.md`. **OP-18 closed as the C3 blocker:** the team-credit half *dissolves* under A2 (credit = own hours worked, no output-decomposition needed); the genuine residue — **labour across co-products — resolves as a declared convention: labour rides the process's own material split** (§3.4a), and **co-product cost is embodied input, never scarcity/yield** (cost ≠ value, A5). §1.1 labour row filled; §3.4a extended to labour + the cost-not-scarcity rule. Resolution note: `00-strategy/OP-18_labour_and_team_credit.md`. Also confirmed by the recursion sim (`06-simulation/RESULTS.md`), which cleared the materials/energy split this now extends.
> **Prior:** v0.5 superseded v0.4 — OP-23 (capital front-loading + holding-time-split), §3.2b (pollution/transport never transfer), §3.3 stock-dependence, §3.6 recycling & end-of-life. Resolution note: `00-strategy/OP-23_capital_and_pollution.md`.
...
> **Companion:** `00-strategy/Aequitas_Objections_v0.6.md` — the objections register. Read alongside §10.
...
- You take on an item's accumulated **property** debit — the embodied *material* you now hold — when you acquire it.
...
**Corollary — pre-Aequitas assets** *(added v0.5)*. A cooperative taking over a 50-year-old hospital cannot reconstruct the architects' fees or the original currency costs. The asset therefore *enters* Aequitas at an estimated or zero creation-cost and accrues history from genesis forward. The pre-genesis past is unrecoverable — the same cutoff, in a new domain.
