...
> **Supersedes:** `99-archive/Aequitas_Overview_v0.6.md`. Two critique-response edits. **(1)** Embeds the **debit-taxonomy schematic** into §1 (the "Solution: Debit" section) — the reviewer's #1 ask was to make the core machinery visual, and §1 is where the plain-language taxonomy lives. **(2) Corrects the disparity-ceiling claim in §1** (critique #9): the ~2.4× cap is no longer presented as flat arithmetic certainty but as a *conditional* result — it depends on sane baselines and on the (unsolved) verification-without-exposure problem (§8). §10's influence bullet aligned. *(Prior: v0.6 folded in the OP-9 socialist-calculation reply — §9's "you can't run an economy without prices" answer; rigorous version `00-strategy/OP-9_calculation_reply.md`.)*
> **Rigorous version:** `00-strategy/Aequitas_Foundations_v0.9.md`. This document is the plain-language companion; where they differ, the Foundations govern.
...
*For the rigorous statement of everything above, with the arguments and the open problems in full: `00-strategy/Aequitas_Foundations_v0.9.md`.*
