...
> **Companion:** `00-strategy/Aequitas_Objections_v0.10.md` — the objections register. Read alongside §10.
...
> **The floor is the denominator of the inequality ceiling** *(new in v0.8)*. Because credit accrues in equally-distributed, non-transferable time (§0), the ratio between the highest and lowest credit-accrual rate is bounded by **`24 h ÷ the network's self-care floor`** — ≈2.4× for a 10 h floor, 3× for 8 h, and so on — against the *unbounded* (Pareto) top tail money produces. It is a small constant, not a fixed one: the floor is a network weighting choice (§6.1b), so the bound is per-floor and *within-model* (across networks, each counterparty re-computes through its own model, §6.4b). This is the **disparity ceiling** — the strongest defensive result the theory reaches — and it is a consequence of the unit of account, not of any rule that polices it. Its formal statement and simulation are the open work (Objections; the debit-tolerance dynamics are OP-4).
...
Ranked by how load-bearing they are. Full detail in `00-strategy/Aequitas_Objections_v0.10.md`.
