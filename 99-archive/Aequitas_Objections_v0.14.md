...
> **Version:** 0.14 · **Date:** 2026-08-11
> **Supersedes:** `99-archive/Aequitas_Objections_v0.13.md`. **Conformed to Foundations v0.13 (pledge-mechanics correction) + presentation cleanup.** A pledge is a *revocable grant of debit-room* (virtual credit backed 1:1 by the pledger's own balance), not a costly debit-absorption and not a promise to buy — corrected in B8, B3, B10 and §C. One consequence for the register: the influence back-door's **"self-starving" brake is retracted** — it assumed pledging *costs* the pledger debit-room, which under v0.13 it no longer does, so the OP-1 residual leans only on IC-7 and the wrecked ratio. Inline version-tags stripped, a Contents list added, and the in-document change log (§E) moved to `Aequitas_Objections_CHANGELOG.md`.
> **Tracks:** `Aequitas_Foundations_v0.13.md` · `Aequitas_EventLog_v0.6.md` · `OP-18_labour_and_team_credit.md` · `OP-23_capital_and_pollution.md` · `OP-17_coproduct_allocation.md` · `06-simulation/RESULTS.md` · `06-simulation/DISPARITY_CEILING.md` · `06-simulation/scenario_suite_METHOD.md`
...
**The one surviving residual.** Realized credit → pledging-power (influence) is measured in *gross hours*, so a consumption-indifferent zealot could collude to fake gross hours and pump influence — bounded by IC-7 (24-hour cap) (24 h/day) and paid in a wrecked ratio. *(The earlier "self-starving" brake no longer applies: under Foundations v0.13 a pledge is a revocable grant of debit-room that does not cost the pledger anything, so pledging does not drain the influence-pumper's own room.)* **This is an OP-1 influence question, not a credit-accounting flaw.** Tests owed: §C.
