...
> **Supersedes:** `99-archive/Aequitas_Foundations_v0.12.md`. **Pledge-mechanics correction (§6.2b, §6.4, §6.4a).** A pledge does **not** spend or transfer the pledger's credit and is **not** a promise to buy. It is a *revocable grant of debit-room* — virtual credit conferred on the receiving person or cooperative, backed 1:1 by the pledger's own balance, retractable at any time. The pledger's only cost is that committed pledging-power is unavailable to pledge elsewhere; their credit never moves. Purchase remains a separate act: whoever takes possession of the resulting good uses their *own* debit-room for it (§3.2). Full change log in §12.
...
> **Companion:** `00-strategy/Aequitas_Objections_v0.14.md` — the objections register. Read alongside §10.
...
1. **Community pledges grant the holders debit-room to carry the creation-cost.** A pledge is **not** costly to the pledger and does **not** draw the creation-cost down: it is a *revocable grant of debit-room* — virtual credit conferred on the receiving cooperative, backed 1:1 by the pledger's own balance (§6.4). The pledger's credit never moves and is never earmarked; their only cost is that committed pledging-power cannot be pledged elsewhere. To the receiver it acts as virtual credit, defraying the bite of the fixed cost. Pledges are simultaneously the **construction authorization** and the **demand brake** — a facility is built at the scale the community will pledge for, the same "pledging supplies the natural limit" logic as §6.2. *(Hospital: a 100k creation-cost with 50k pledged still sits at 100k on the asset — but 50k of pledge-granted debit-room means only 50k of it effectively restricts the holders.)*
2. **The full creation-cost is holding-time-split among the asset's holders** — pledges cushion the bite, they do not shrink the debit (nothing may vanish, A1). Each holder's permanent share = **their holding-duration ÷ total holding-duration over the asset's whole life** (§1.1). Pledges are **retractable at any time**: withdraw them and the holders' debit-room contracts, re-exposing the full holding-time share. A cooperative therefore leans on pledges hardest at start-up and aims to reach a scale where losing any single pledge is survivable — the pledger's ongoing approval of its work is what keeps the cushion in place.
...
**Why pledges must be exactly 1:1.** A pledger's outstanding pledges are debit-room they confer on receivers, and all of it must be backed by their own earned credit; the 1:1 cap holds total pledging-power to that backing. Let it exceed and you get **fractional-reserve pledging** — more debit-room granted across the network than the grantors' credit can stand behind, so a receiver leans on a cushion that was never really there and a wave of retractions leaves it exposed. This is a solvency constraint, not a preference. It also happens to be the only stationary value: pledging power created per period is *kL* and consumed at most *L*, so any *k* > 1 diverges until pledges filter nothing, and any *k* < 1 shrinks the directed economy to zero.
...
**Approval never gates credit — but *verification* gates its realization**. The work is **always recorded**: an event is logged the moment work is done, so origin closure holds and unpledged wheat still has a grower (A7, IC-3). What a pledge buys is **authorization and demand-room** for the work — a revocable grant, not a guaranteed sale; whoever ultimately takes the resulting good uses their own debit-room to hold it (§3.2, §6.4a). But a recorded credit **realizes** — begins counting toward the worker's position — only when the output is **verified**, exactly as A7 already gates an estimated position on observation. This is *verification, not approval*: no committee judges the work worthy; the trigger is objective evidence the output exists. See §6.4a for how, for a physical good, that verification *is* the hand-off.
...
- **Pledged production** (a run backed by pledges): the pledges **soften the run's demand risk** — they signal committed demand and grant the producer debit-room to carry the work while it is in progress. But a pledge is **revocable and is not a promise to buy** (§6.4), so it *reduces* risk rather than eliminating it: pledgers can withdraw, and taking the finished good is always a separate possession act on the buyer's own debit-room. The producer's exposure is therefore lowest for a mature, well-regarded run and highest at start-up — which is exactly the incentive to keep pledgers satisfied. Most work is of this kind.
...
> **Now simulated**. The ceiling has been demonstrated in an agent-based model — `06-simulation/disparity_ceiling_sim.py` (N = 200,000, gate `D_i ≤ ρ·C_i`, credit ∈ `[F,24]` h/day), companion `06-simulation/DISPARITY_CEILING.md`. Three results, all within-model:
>
...
Ranked by how load-bearing they are. Full detail in `00-strategy/Aequitas_Objections_v0.14.md`.
