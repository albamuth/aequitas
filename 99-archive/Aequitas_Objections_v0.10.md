...
> **Version:** 0.10 · **Date:** 2026-08-09
> **Supersedes:** `99-archive/Aequitas_Objections_v0.9.md` — **the OP-9 write-up fold.** **OP-9 (calculation reply) / P5 (preference revelation) downgraded 🟠→🟢** ("substantially answered, needs writing up" → **written up**): the four-move reply (cost≠value · pledges reveal demand · tractability cited · scarcity-as-debit without an objective function) now lives in the standalone `00-strategy/OP-9_calculation_reply.md`, with the plain-language version folded into Overview v0.6 §9. **OA8 rewritten** to point at the standing doc and record the one unclosed edge — the scarcity-as-debit and Hayek-tacit-knowledge residues both **terminate in OP-10 (weighting governance)** and are registered there, not claimed closed. No axiom change; no new hole. Prior fold (v0.9) below.
> **Prior (v0.9):** the work-definition session. The self-care / definition-of-work cluster stress-tested to **PASSES** (**B11**): *as an instance of OP-10/OP-22, not a new hole* — network weighting-pluralism is legitimate (A6/§3), and the anti-arbitrage guard is **counterparty re-computation** (comparison, never conversion), which depends on OP-22 (audit disclosure). **OP-4 (debit tolerance) promoted** (now the denominator of the disparity ceiling; no global debit:credit ratio exists — §3.5/A8 forbid it); **OP-1 (service → influence)** gains the **universal-basic-voice** result (self-care credit bounds influence disparity to `24h ÷ floor`); **OP-14 (cohort shopping)** gains self-care-floor / routing shopping; **OP-8 (feedback firewall)** sharpened (feedback ≠ verification); **OP-10 (weighting governance)** gains its highest-leverage instance — the self-care constant (universal + influence-bearing).
> **Tracks:** `Aequitas_Foundations_v0.9.md` · `Aequitas_EventLog_v0.5.md` · `OP-18_labour_and_team_credit.md` · `OP-23_capital_and_pollution.md` · `OP-17_coproduct_allocation.md` · `06-simulation/RESULTS.md`
