...
> **Depends on:** `Aequitas_Foundations_v0.4.md` A1–A8
> **Supersedes:** `99-archive/Aequitas_EventLog_v0.2.md`. See §15.
...
**The distinguishing property is structural, not a flag: a `Pledge` transfers debit and a `Signal` does not.** A signal has no `hours` field and never touches a ledger.
...
**IC-9 — Pledge discharge is a transfer.** When pledged work occurs, the resulting property debit moves to the pledger under the ordinary custody rules (IC-5).
...
### 5.1b Why credit remains ungated

**A pledge never gates credit.** An agent is credited for work performed whether or not anyone pledged for it. Gating credit on approval would contradict A7 and reproduce the origin-closure failure that the A7 v0.2 amendment repealed.
...
**IC-3 — Origin closure (backward).** Every parcel traces backward to a reservoir extraction. **No parcel may appear without ancestry.**
...
**Nothing is added to the schema, and that is the finding.** An event already records several `outputs`, and §3 already forbids the event from carrying any weight or cost. The split is therefore computed **at projection time**, from the event's `process.taxonomy_ref` plus the published process-energetics model for that classifier — exactly where the mitigation-cost weights already live.
...
| **Wash-pledging rings** | A1 still forces real matter to move, and the goods carry real property debit someone must hold | Mitigated |
