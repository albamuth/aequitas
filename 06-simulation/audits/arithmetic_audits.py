...
FIVE MODELLING RULES THE LOG OBEYS (version history is in CHANGELOG.md):

  * Survival is entity-record continuity, not a within-event "same id on both
    sides" match. Parcels are persistent records with a lifecycle.
  ...
  * A pledge is a PERMANENT, non-revocable grant of debit-room, drawn 1:1 from the
    ...
    It moves no debit by itself and cannot be withdrawn; an undischarged pledge
    that reaches expiry BURNS (its budget is lost, never returned). When the
    summoned work yields a held object, the object's debit follows possession
    (IC-5) to *whoever accepts it* -- NOT necessarily the pledger. A pure
...
# The solver lives in a sibling project directory (06-simulation/allocation-engine).
sys.path.insert(0, os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "allocation-engine"))

from recursion_convergence import Economy, build_forward, solve_direct, spectral_radius  # noqa: E402
...
class Parcel:
    ...
    origin_reservoirs: tuple[str, ...] = ()   # every reservoir input, if created straight from reservoirs
        ...
        for ev in self.events:
            ...
            for f in ev.outputs:
                if f.is_parcel and f.endpoint_id not in in_ids:
                    ...
                    # parcel ancestry (a genesis entry has neither). ALL of the
                    # event's reservoir inputs are recorded, not just the first:
                    # IC-3 has to check every one of them against the registry.
                    origin_res: tuple[str, ...] = ()
                    if not origin_gen and not in_ids:
                        origin_res = tuple(g.endpoint_id for g in ev.inputs
                                           if not g.is_parcel)
                    self.parcels[f.endpoint_id] = Parcel(
                        id=f.endpoint_id, substance=f.substance,
                        created_by=ev.id, created_at=ev.start,
                        origin_reservoirs=origin_res, origin_genesis=origin_gen,
                    )
...
def check_ic3_origin_closure(log: LogState):
    ...
    recursed; a genesis root, or a creating event whose reservoir inputs are
    ALL in the registry, is a valid terminus. A parcel with no creating event,
    a cycle with no root, or any 'reservoir' input that is not in the registry,
    fails."""
    def traces(parcel_id: str, seen: set[str]) -> bool:
        ...
        if p.origin_reservoirs:
            # EVERY reservoir the creating event drew on must be named in the
            # registry -- one unregistered draw is one unclosed origin.
...
def computed_theta(ev: Event, dim: str,
    ...
    if len(parcels) < 2:
        return None
    mass_flows = [f for f in parcels if f.unit == "kg"]
    mass_total = sum(f.magnitude for f in mass_flows)
    mass_split = ({f.endpoint_id: f.magnitude / mass_total for f in mass_flows}
...
def violate_ic1(log):
    ...
    _find(log, "E6").outputs[0].magnitude = 10.5      # bread 9.5 -> 10.5, +1.0 kg
    return log
...
def _tests() -> None:
    ...
    assert log.parcels["P:tool"].origin_genesis and not log.parcels["P:tool"].origin_reservoirs
