...
- **Credit is time (A2).** A person's credit accrues at a rate `c_i` = hours credited per day. It is `c_i = F + w_i`, where **F** is the [self-care floor](../../01-wiki/pledge-and-signal.md) credited to *every verified living human* by proof-of-life (Foundations §6.1b / §7.5) and `w_i ∈ [0, 24−F]` is discretionary worked hours.
