"""
track3_exiobase.py -- foreign labour-hours embodied in US household consumption,
from the EXIOBASE 3 global MRIO (2022, pxp).

NOT THE TRACK 3 ANSWER. Read this before citing anything below.

  This script is SUPERSEDED for Track 3. The Track 3 result comes from
  track3_imports.py, which TRACK3.md names and which caches its answer in
  track3_result.json. Cite that one.

  History, because the line below used to say the opposite and confused two
  readers. Written 2026-08-10, this script replaced a wage-and-margin ballpark
  that track3_imports.py contained AT THAT TIME. On 2026-08-17 track3_imports.py
  was rebuilt to do the full EXIOBASE solve itself, absorbing this method and
  adding a cached result and self-tests. So the "replaces track3_imports.py"
  claim is true only of a version of that file which no longer exists.

  WHY IT IS STILL HERE, and not in archive/: average_footprint.py imports its
  L @ Y_us_households machinery to answer a DIFFERENT question -- the
  environmental footprint (CO2, materials, land, water, energy), not labour.
  This file is retained as that shared plumbing. It is not a second opinion on
  Track 3 and the two figures should never be presented side by side.
