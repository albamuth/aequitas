...
**2. There is no disk in this run.** Both passes read flat in-memory arrays. **A real log is serialised and stored, both passes read the same bytes, and shared I/O would push the ratio toward 1.0.** How far is unmeasured. **This is the largest gap in the result and it should be stated wherever the figure is quoted.**
