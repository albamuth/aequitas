...
> **Companion:** `00-strategy/Aequitas_Objections_v0.18.md` — the objections register. Read alongside §10.
...
- [11. First Foothold — how adoption plausibly starts](#11-first-foothold--how-adoption-plausibly-starts)
...
**The accounting is identical in both.** Conservation still holds, the integrity constraints still recompute, the residual rule still runs, the disparity ceiling still binds. **Nothing in §§1–10 changes.**
...
> **The dial test is the standing screening question for anything proposed for these documents, and §9 states what that leaves behind:** the implementer-facing output is a set of **conformance requirements**, never an architecture. **What must be true, never how to build it.**
...
Two networks do not negotiate a person's balance. **They agree on how a ledger is calculated, and then they both get the same answer** — which is what §7.5 condition 5 means by *arriving at the same ledger for the same person*. Consensus is at the **method** level. Numbers follow.
...
**Nobody imposes a penalty. The books are simply correct, and being correct is the penalty.** Essentials are untouched throughout — §7.5 restricts non-essentials only, and does so for the fraudster on exactly the terms it does for anyone mis-measured.
...
**Aequitas has to be usable by someone who still uses money, as an alternative that does not exclude them.** §11 and §5.2 both assume this and neither works it out. This section works it out.
...
**The attack worth taking seriously.** Aequitas prices at cost (A5) and the money economy prices at market, so anything whose market price exceeds its Aequitas cost is an arbitrage: buy inputs inside cheaply, sell outputs outside at a profit, and disregard a ledger you do not care about.
...
> **⚠️ Repeat-shell entities.** A chain — buy inside, sell to a shell inside, let the shell sell outward and absorb the ruined ratio — is bounded for *persons* by **one verified human, one account** (§5.1), and each shell is a real person burning their own standing in public. **It is not obviously bounded for entities**, because a co-op is not a person. **This is a C6 (identity) question and a sibling of OP-25 (illicit dumping). Registered, not solved.**
...
> 5. **Networks that count the same person are compatible only if they arrive at the same ledger for that person.** A person may legitimately hold an account on more than one network (§5.3a). That is not two credits — **it is one life, counted twice.** Two networks counting the same person are counting the same thing, so **compatibility *means* they can re-derive each other's numbers and land on the same ledger** (OP-14, comparison never conversion). Where they can, there is **one** ledger seen from two places, the self-care floor is credited **once**, and there is nothing to add up.
>
>    **Where they cannot, they are not compatible**, and their credits do not interoperate — goods and claims do not cross. What is left in that case is **not a breach of the ceiling but a gap in coverage**: each network holds a *partial* record of that person's material life, and §7.4's extent rule requires each to say so. **This is a coverage question (§5.1c), not a bound that has been exceeded.**
>
>    *Federation is therefore not a patch bolted onto the ceiling — it is what interoperation is.*
>
> So the honest statement is: **if OP-22 is solved and floors stay in-band, then the accrual ceiling is `24 h ÷ floor` across any set of networks compatible enough to interoperate — which is what compatibility means.** Incompatible networks do not sum; they simply do not trade, and each holds a partial record it must declare as partial. It remains the **disparity ceiling** — the strongest defensive result the theory reaches — a *conditional* result whose debit-tolerance dynamics are OP-4 (debit tolerance).
...
That is §1.2 turned on this project's own output. Aequitas is an economic system, and the work here is to state it and test it against scenarios. **It is not a data-architecture project.** A schema, a storage design, a transport protocol, and a choice of cryptography are praxis, and they belong to whoever implements.

**But implementers are still a real audience with a real need.** Someone building a trust network has to be able to read these documents and know **which system they are implementing.** That need is met by **conformance requirements** — a list of things that must hold — never by an architecture.

> **The standing screening question for anything proposed for these documents is §1.2's dial test: if a principle survives at both ends of a dial, the dial is not part of the principle.**
>
> Conservation of mass and energy survives at both ends of every dial. **A field name does not.**

### What a conformance statement carries
...
1. **Foundations** *(this document)* — the system itself: axioms, mechanisms, and what follows from them. **Audience: anyone.**
2. **Conformance requirements** — the table above, drawn out and made precise enough to check an implementation against. **Audience: implementers.** Not a schema, not a protocol, not a product.
3. **Academic paper** — engages Marx / Hayek / Ostrom; must answer the socialist-calculation critique head-on. Lead with: theory of *cost* not value; Ellerman on attribution; Cockshott & Cottrell on tractability; pledges as a decentralized answer to preference revelation. **Add: joint production solved by process physics rather than by convention (§3.4a) — this is the reply to Sraffa/Steedman and to ISO 14044 simultaneously.**
...
Ranked by how load-bearing they are. Full detail in `00-strategy/Aequitas_Objections_v0.18.md`.
...
- **OP-25 (illicit dumping) — Illicit end-of-life dumping.** §3.6 prices *lawful* disposal correctly; abandoning an object in the environment to escape its end-of-life debit is a Level-2 trust-and-provenance attribution problem. *v0.19 adds a sibling from §5.5: **repeat-shell entities.** One verified human is one account, so a chain of shells is a chain of real people burning their own standing in public — but **a co-op is not a person**, and nothing obviously bounds how many can be created. **A C6 (identity) question, and it should be worked with this one.***

**Closed**
- ✅ ~~**OP-27 (parallel implementation) — Using Aequitas while the world still uses money.**~~ **Closed 2026-08-23 (§5.5), ruled and stress-tested.** Both directions across the boundary are deliberately costly; a money-made good is dark until sold in and clears via a **published template**; selling out keeps the debit and reads as a **gift**, because money is not matter (A1). **No axiom moved and no cross-boundary rule was needed.** Extraction self-limits through the extractor's own ratio gate, and **money cannot buy standing at any scale** — the workers are credited the hours, not whoever paid them. **Two items registered rather than solved:** repeat-shell **entities** (below, with OP-25) and **template capture** (above, with OP-24). Paper: `00-strategy/OP-27_parallel_implementation.md`.
...
<!-- tag: fnd-s11 -->
## 11. First Foothold — how adoption plausibly starts

> **Scope note, and it governs the whole section.** This is a reading of the historical record on how a system like this gets a foothold. **It is not a deliverable of this work.** Under §1.2 and §9, building a demonstration is praxis a project **may** choose to take on; nothing here is owed, and no result in §§1–10 waits on it. What follows is evidence about adoption, offered to whoever decides to try.
