...
> **Reads against:** `Aequitas_Foundations_v0.19.md` A1, A7, §3.2, §3.2b, §5.1, §5.1a, §5.1b, §5.1c, §5.1d, §5.2, §5.3a, §6.2a, §7.5 · `06-simulation/q4_locked_ledgers.py` · `06-simulation/residual_unravelling.py`
...
**And Foundations already knew this.** [`q4_locked_ledgers.py`](../sims/q4_locked_ledgers.py), quoted in §7.5: *"even fully divesting material property does not save them (permanent consumption debit, §3.2)."*
...
[`residual_unravelling.py`](../sims/residual_unravelling.py), recorded in §5.3a residue (b):
...
5. **A wealth distribution with a real tail** — the SCF-2022 and Forbes material-only distribution already in [`q4_locked_ledgers.py`](../sims/q4_locked_ledgers.py). **And the 1% coverage cut must be OFF**, or the tail is invisible by construction.
