...
> #### ⚠️ What convergence costs — and this must not be quietly accepted
>
> **Convergence removes exit, and exit was doing real work.**
>
> §5.3a prices opacity, and §5.3b prices hoarding, by letting members and counterparties *choose a different network*. **That check exists only while there is a different network to choose.** As the ecosystem converges, the discount mechanism loses its teeth in exactly the same measure as the seams close.
>
> **And OP-10 becomes singular.** One agreed method set, governing everyone, is the largest capture surface this project can produce. §3.3a's defences — rival-sector audit, two unaffiliated replications, public membership as a screen — were written for a world of competing networks and **do not obviously survive their own success.**
>
> **The trade is not avoidable by cleverness. It is structural:** *a global bound requires a global method, and a global method removes network-shopping as an escape.* You cannot hold both the worldwide ceiling and the ability to walk away from the algorithm that sets it.
>
> **One distinction may carry most of the weight, and it is worth testing rather than assuming.** What merges is the **method**, not necessarily the **custodian**. Many holders can share one algorithm set the way many operators share one network protocol — one accounting *domain*, many *databases*. If that holds, convergence costs the exit-based checks but **not** the horizontal protection against information capture (§5.3b). If it does not hold — if convergence means one operator — then the single largest concentration of information in the system is the end state of its own design. **Unresolved. Registered against OP-10 and C2.**
