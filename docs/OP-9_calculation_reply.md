...
This is an empirical claim and it has been **empirically answered.** [Cockshott & Cottrell](https://en.wikipedia.org/wiki/Towards_a_New_Socialism) demonstrated that in-kind calculation at national scale is computationally feasible using **sparse-matrix methods** over a national input-output structure, on ordinary hardware. People ran the arithmetic; it runs. Aequitas's own recursion-convergence sim (`06-simulation/RESULTS.md`) is a second, independent instance: the per-product allocation is a non-negative Neumann series that converges in ~10 s for a 10⁴-node economy.
