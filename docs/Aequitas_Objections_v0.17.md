...
| | Item | Status |
|---|---|---|
| 🔴 | **OP-10 (weighting governance) / P8 (weighting governance) — Weighting-model governance** | Open. One side entrance closed (§3.2a); cost constants have a mechanism (§3.3a); the general problem stands. **Now the top blocking problem — and §5.3c makes it worse over time.** Interoperating requires consensus on the calculating algorithms, and the expected trajectory is **convergence toward a single network**. That means **one method set governing everyone: the largest capture surface the project can produce.** §3.3a's defences — rival-sector audit, two unaffiliated replications, public membership as a screen — were written for a world of competing networks and **do not obviously survive their own success**. **The trade is structural, not a design flaw: a global bound requires a global method, and a global method removes network-shopping as an escape.** |
