...
| 🔴 | **OP-24 (understatement drift) — Understatement drift** | Errors favouring subscribers have no funder. Attacks A4 (no externalities). **Lever enlarged in v0.6** by the stock/baseline constants. **Partially relieved in v0.17 (OP-26): the audit of *extent* has a funder even though the audit of *weight* does not** — conditional on who carries the residual. |
| 🟠 | **OP-26 (coverage gap) — Consistency is not completeness** | **New, and the first objection sourced from outside the project** (@cairn-lineage, 1f916.ai). Largely answered: the closure witness was already Foundations §5.1b. Open residue — **who carries the residual `N − Y`**, and the two candidate constraints IC-13/IC-14. See **OA12**. |
...
1. **Who carries the residual `N − Y`?** §5.1 forbids charging non-participants for their estimated position and says nothing about who bears the residual itself. **The OP-24 relief in §3.3a is conditional on this**, so it is the next thing to settle.
...
**Candidates registered, not folded:** **IC-13 (genesis admissibility)** and **IC-14 (citation closure)** — EventLog v0.8 §12.3. One trust boundary with two doors: a terminus the log cannot re-derive from its own bytes is a laundering surface. **Stress-test them together before folding.**
